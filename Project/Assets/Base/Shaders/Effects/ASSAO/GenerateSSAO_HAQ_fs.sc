...
uniform vec4 adaptiveSampleCountLimit;
...
SAMPLER2D(s_importanceMap, 6);
SAMPLER2D(s_averageImportance, 7);
SAMPLER2DARRAY(s_SSAOMapBase, 8);
...
void GenerateSSAOShadowsInternal(out float outShadowTerm, out vec4 outEdges, out float outWeight, const vec2 SVPos, const vec2 TexCoord)
{
    ...
    vec3 neighbourNormalL = LoadNormal(fullResCoord + vec2(-2.0 * viewportPixelSize.x,  0.0 ));
    vec3 neighbourNormalR = LoadNormal(fullResCoord + vec2( 2.0 * viewportPixelSize.x,  0.0 ));
    vec3 neighbourNormalT = LoadNormal(fullResCoord + vec2( 0.0, -2.0 * viewportPixelSize.y ));
    vec3 neighbourNormalB = LoadNormal(fullResCoord + vec2( 0.0,  2.0 * viewportPixelSize.y ));
    ...
    float importance = texture2D(s_importanceMap, fullResCoord).x;
    ...
    vec2 baseValues = bgfxTexture2DArray(s_SSAOMapBase, vec3(TexCoord, passNumber.x)).xy;
    ...
    float avgTotalImportance = texture2D(s_averageImportance, vec2(0.0, 0.0));
    float importanceLimiter = saturate(adaptiveSampleCountLimit.x / avgTotalImportance);
    ...
    {
        additionalSampleCountFlt -= 1.0;
        float weightMod = saturate(additionalSampleCountFlt * blendRangeInv);
        SSAOTap(obscuranceSum, weightSum, i, rotScale, pixCenterPos, negViewspaceDir, pixelNormal, TexCoord, mipOffset, falloffCalcMulSq, weightMod, normXY, normXYLength);
    }
