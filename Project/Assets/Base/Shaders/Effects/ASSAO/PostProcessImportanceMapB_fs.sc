...
void main()
{
    ...
    vec4 vals;
    vals.x = texture2D(s_importanceMap, v_texCoord0 + vec2(-halfPixel.x, -halfPixel.y * 3.0)).x;
    vals.y = texture2D(s_importanceMap, v_texCoord0 + vec2( halfPixel.x * 3.0, -halfPixel.y)).x;
    vals.z = texture2D(s_importanceMap, v_texCoord0 + vec2( halfPixel.x,  halfPixel.y * 3.0)).x;
    vals.w = texture2D(s_importanceMap, v_texCoord0 + vec2(-halfPixel.x * 3.0,  halfPixel.y)).x;
