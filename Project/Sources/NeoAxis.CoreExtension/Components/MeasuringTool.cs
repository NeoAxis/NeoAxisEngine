...
namespace NeoAxis
{
	...
	/// </summary>
	[AddToResourcesWindow( @"Base\Scene objects\Additional\Measuring Tool", 0 )]
	public class MeasuringTool : ObjectInSpace
	{
		/// <summary>
		...
		{
			var point = CreateComponent<MeasuringToolEndPoint>();
			...
			var text = CreateComponent<Text2D>();
		...
		{
			var point = GetComponent<MeasuringToolEndPoint>();
			if( point != null )
				point.Transform = new Transform( Transform.Value.Position + new Vector3( 1, 0, 0 ), Quaternion.Identity );
		...
		public MeasuringToolEndPoint GetEndPoint()
		{
			return GetComponent<MeasuringToolEndPoint>();
		}
		...
		public delegate void UpdateTextDelegate( MeasuringTool sender, ref string text );
		public event UpdateTextDelegate UpdateText;

		void UpdateTextComponent()
		{
			var text = GetComponent<Text2D>();
			if( text != null )
			{
	...
	/// <summary>
	/// End point for <see cref="MeasuringTool"/>.
	/// </summary>
	public class MeasuringToolEndPoint : ObjectInSpace
	{
		protected override void OnTransformChanged()
		{
			base.OnTransformChanged();

			var start = Parent as MeasuringTool;
