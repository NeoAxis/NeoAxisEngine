...
namespace NeoAxis.Editor
{
	...
	/// </summary>
	public class PathfindingTestMode : SceneEditor.WorkareaModeClassScene
	{
		Pathfinding owner;

		public PathfindingTestMode( SceneEditor documentWindow, Pathfinding owner )
			: base( documentWindow )
		{
			this.owner = owner;
		}
		...
		protected override bool OnKeyDown( Viewport viewport, KeyEvent e )
		{
			if( e.Key == EKeys.Escape || e.Key == EKeys.Space )
			{
				...
				ObjectEx.MethodInvoke( DocumentWindow, "ResetWorkareaMode" );
		...
		{
			var context = new SceneEditor.GetMouseOverObjectToSelectByClickContext();
