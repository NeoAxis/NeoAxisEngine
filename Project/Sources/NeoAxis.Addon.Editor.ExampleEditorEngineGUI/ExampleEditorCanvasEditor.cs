...
namespace NeoAxis.Addon.ExampleEditorEngineGUI
{
	public partial class ExampleEditorCanvasEditor : CanvasBasedEditor
	{
		...
		void ShowContextMenu()
		{
			...
			{
				var item = EditorContextMenu.NewItem( "Item 1", delegate ( object s, EventArgs e2 )
				{
			...
			items.Add( EditorContextMenu.NewSeparator() );

			{
				var item = EditorContextMenu.NewItem( "Item 2", delegate ( object s, EventArgs e2 )
				{
				} );
				item.Enabled = false;
				items.Add( item );
			...
			{
				var item = EditorContextMenu.NewItem( "Item 3", delegate ( object s, EventArgs e2 )
				{
				} );
				item.Enabled = false;
				items.Add( item );
			...
			//	{
			//		EditorAPI.EditorActionClick( IEditorAction.HolderEnum.ContextMenu, "Duplicate" );
			//	} );
			//	item.ShortcutKeyDisplayString = EditorActions.GetFirstShortcutKeyString( "Duplicate" );
			//	item.Enabled = EditorAPI.EditorActionGetState( IEditorAction.HolderEnum.ContextMenu, "Duplicate" ).Enabled;
			//	items.Add( item );
			//}
			...
			//	{
			//		EditorAPI.EditorActionClick( IEditorAction.HolderEnum.ContextMenu, "Delete" );
			//	} );
			//	item.Enabled = EditorAPI.EditorActionGetState( IEditorAction.HolderEnum.ContextMenu, "Delete" ).Enabled;
			//	items.Add( item );
			//}
		...
		void CalculateCellSize()
		{
			...
			if( SystemSettings.DPIScale > 1 )
			{
				cellSize *= SystemSettings.DPIScale;
				cellSize = (int)cellSize;
			}

			//float dpi = SystemSettings.DPIScale * 96;//EditorAPI.DPI;
			//if( dpi > 96 )
			//{
			//	cellSize *= dpi / 96;
			//	cellSize = (int)cellSize;
