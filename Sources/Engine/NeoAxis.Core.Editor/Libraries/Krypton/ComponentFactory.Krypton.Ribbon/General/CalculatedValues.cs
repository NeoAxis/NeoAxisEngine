...
namespace Internal.ComponentFactory.Krypton.Ribbon
{
    ...
    internal class CalculatedValues
    {
        ...
        private static readonly int FONT_HEIGHT_EXTRA =         DpiHelper.Default.ScaleValue(2)+1;   // Always need to add 2 pixels for it to draw
        private static readonly int DIALOG_MIN_HEIGHT =         12; /*dont scale image now*/  // Button image = 8 + 2 content gap + 2 for button border + 2 for outside gap
        private static readonly int GROUP_LINE_CONTENT_MIN =    18; /*dont scale image now*/  // Small image = 16 + 2 content gap
        private static readonly int GROUP_LINE_CONTENT_EXTRA =  (int)(4 / DpiHelper.Default.DpiScaleFactor);   // 2 content gap + 2 for button border
        private static readonly int GROUP_INSIDE_BOTTOM_GAP =   DpiHelper.Default.ScaleValue(1);   // 1 pixel between last group line and the group title
        private static readonly int GROUP_TOP_BORDER =          DpiHelper.Default.ScaleValue(2);   // 2 pixel border for top edge of a group
        private static readonly int GROUPS_TOP_GAP =            DpiHelper.Default.ScaleValue(3);   // Space between top of a group and inside edge of borders area
        private static readonly int GROUPS_BOTTOM_GAP =         DpiHelper.Default.ScaleValue(2);   // Space between bottom of group and bottom of the borders area
        private static readonly int TABS_TOP_GAP =              DpiHelper.Default.ScaleValue(5);   // 4 padding at top of tab text and 1 extra for the bottom
        private static readonly int KEYTIP_HOFFSET =            DpiHelper.Default.ScaleValue(16);  // Horizontal distance to offset keytips for group items
        private static readonly int KEYTIP_VOFFSET_LINE2 =      DpiHelper.Default.ScaleValue(1);   // Vertical distance to offset keytips on group line 2
        private static readonly int KEYTIP_VOFFSET_LINE4 =      DpiHelper.Default.ScaleValue(8);   // Vertical distance to offset keytips on group line 4
        private static readonly int KEYTIP_VOFFSET_LINE5 =      DpiHelper.Default.ScaleValue(8);   // Vertical distance to offset keytips on group line 5
        #endregion
