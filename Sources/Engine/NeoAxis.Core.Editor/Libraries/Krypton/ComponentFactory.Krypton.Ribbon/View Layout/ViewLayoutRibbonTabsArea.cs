...
namespace Internal.ComponentFactory.Krypton.Ribbon
{
    ...
    internal class ViewLayoutRibbonTabsArea : ViewLayoutDocker
    {
        ...
        private bool IsBackstageAppMenuMode()
        {
            return _ribbon.ApplicationButtonDropDownMenu != null;
        }

        public /*private */ void OnAppButtonClicked(object sender, EventArgs e)
        {
            // We do not operate the application button at design time
            if ( _ribbon.InDesignMode )
            {
                if ( !IsBackstageAppMenuMode() ) OnClassicAppMenuDisposed( this, EventArgs.Empty );
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        if ( IsBackstageAppMenuMode() )
                            CreateAndShowBackstageAppMenu();
                        else
                            CreateAndShowClassicAppMenu();
        ...
        private void CreateAndShowBackstageAppMenu()
        {
            _appMenu = _ribbon.ApplicationButtonDropDownMenu;
            _appMenu.VisibleChanged += new EventHandler(OnBackstageAppMenuVisibleChanged);
            _appMenu.BringToFront();
            _appMenu.Show();
        }

        private void OnBackstageAppMenuVisibleChanged( object sender, EventArgs e )
        {
            var appMenu = _appMenu as BackstageAppMenu;
            ...
            if (!appMenu.Visible)
            {
                ...
                if ( appMenu != null )
                {
                    ...
                    appMenu.VisibleChanged -= new EventHandler(OnBackstageAppMenuVisibleChanged);
                    ...
                    if ( appMenu.CloseReason.HasValue )
                        closeReason = appMenu.CloseReason.Value;
        ...
        private void CreateAndShowClassicAppMenu()
        {
            ...
            _appMenu.Disposed += new EventHandler(OnClassicAppMenuDisposed);
           ...
           ((VisualPopupAppMenu)_appMenu).Show(appRectShow);
        }

		private void OnClassicAppMenuDisposed(object sender, EventArgs e)
        {
            var appMenu = _appMenu as VisualPopupAppMenu;
            ...
            {
                // Unhook from control, so it can be garbage collected
                appMenu.Disposed -= new EventHandler(OnClassicAppMenuDisposed);
            ...
            if (!_ribbon.IsDisposed)
            {
                ...
                if (!_ribbon.InDesignMode)
                {
                    ...
                    if (sourceContent != null)
                    {
                        ...
                        screenRect.Height -= 20; //TODO: Fix DPI ?
