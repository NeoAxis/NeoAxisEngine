...
namespace Internal.ComponentFactory.Krypton.Toolkit
{
	...
	public class KryptonMessageBox : KryptonForm
	{
		...
		int GAP { get { return (int)( 13.0f * DpiHelper.Default.DpiScaleFactor ); } }
		int GAP2 { get { return (int)( 8.0f * DpiHelper.Default.DpiScaleFactor ); } }
		//private static readonly int GAP = 16;//10;
		//private static readonly int GAP2 = 10;
								  ...
								  HelpInfo helpInfo )
		{
			...
			if( KryptonDarkThemeUtility.DarkTheme )
				_messageText.StateCommon.TextColor = Color.FromArgb( 230, 230, 230 );

			var color = KryptonDarkThemeUtility.DarkTheme ? Color.FromArgb( 40, 40, 40 ) : Color.FromArgb( 255, 255, 255 );
			_panelMessage.StateCommon.Color1 = color;
			_panelMessageIcon.StateCommon.Color1 = color;
			_panelMessageText.StateCommon.Color1 = color;
		}
												 ...
												 HelpInfo helpInfo )
		{
			...
			if( ( helpInfo != null ) || ( ( options & ( MessageBoxOptions.ServiceNotification | MessageBoxOptions.DefaultDesktopOnly ) ) == 0 ) )
			{
				...
				if( owner == null )
				{
					...
					//showOwner = EditorForm.Instance;

					//if( Process.GetCurrentProcess().MainWindowHandle != IntPtr.Zero )
					//	showOwner = Control.FromHandle( Process.GetCurrentProcess().MainWindowHandle );
			...
			using( KryptonMessageBox kmb = new KryptonMessageBox( text, caption, buttons, icon, defaultButton, options, helpInfo ) )
			{
				...
				var result = kmb.ShowDialog( showOwner );

				//!!!!betauser
				//activate when from another thread
				if( EngineApp.AfterFatalOperations )
				{
		...
		static Image GetResized( Bitmap source )
		{
			if( DpiHelper.Default.DpiScaleFactor != 1.0f )
			{
				var newSize = new Size( (int)( 32.0f * DpiHelper.Default.DpiScaleFactor ), (int)( 32.0f * DpiHelper.Default.DpiScaleFactor ) );
		...
		private void UpdateIcon()
		{
			...
			case MessageBoxIcon.Question:
				_messageIcon.Image = GetResized( Properties.Resources.help2 );
				//System.Media.SystemSounds.Question.Play();
				break;
			case MessageBoxIcon.Information:
				_messageIcon.Image = GetResized( Properties.Resources.information );
				//System.Media.SystemSounds.Asterisk.Play();
				break;
			case MessageBoxIcon.Warning:
				_messageIcon.Image = GetResized( Properties.Resources.sign_warning );
				//System.Media.SystemSounds.Exclamation.Play();
				break;
			case MessageBoxIcon.Error:
				_messageIcon.Image = GetResized( Properties.Resources.error );
		...
		private Size UpdateButtonsSizing()
		{
			...
			Size button1Size = new Size( 90/*117*/, 26/*32*/ );// _button1.GetPreferredSize(Size.Empty);
			Size maxButtonSize = new Size( button1Size.Width + GAP2, button1Size.Height );
			...
			case MessageBoxButtons.AbortRetryIgnore:
				{
					numButtons++;
					Size button2Size = button1Size;//_button2.GetPreferredSize(Size.Empty);
					maxButtonSize.Width = Math.Max( maxButtonSize.Width, button2Size.Width + GAP2 );
			...
			case MessageBoxButtons.AbortRetryIgnore:
				{
					numButtons++;
					Size button3Size = button1Size;// _button2.GetPreferredSize(Size.Empty);
					maxButtonSize.Width = Math.Max( maxButtonSize.Width, button3Size.Width + GAP2 );
			...
			case MessageBoxButtons.AbortRetryIgnore:
				{
					...
					right -= maxButtonSize.Width + GAP2;
				}
				break;
			}
			...
			if( DpiHelper.Default.DpiScaleFactor >= 1.5 )
				add = (int)( 6.0f * DpiHelper.Default.DpiScaleFactor );

			// Size the panel for the buttons
			_panelButtons.Size = new Size( ( maxButtonSize.Width * numButtons ) + GAP * ( numButtons + 1 ), maxButtonSize.Height + GAP * 2 + add );

			// Button area is the number of buttons with gaps between them and 10 pixels around all edges
			return new Size( ( maxButtonSize.Width * numButtons ) + GAP * ( numButtons + 1 ), maxButtonSize.Height + GAP * 2 + add );
		}
		...
		private void InitializeComponent()
		{
			...
			this._panelMessageText.Location = new System.Drawing.Point( 52, 0 );
			...
			this._messageIcon.Location = new System.Drawing.Point( 16, 16 );
			...
			this._messageIcon.SizeMode = PictureBoxSizeMode.AutoSize;
			// 
			// _panelButtons
			// 
			...
			this.TopMost = false;//!!!!betauser = true;
