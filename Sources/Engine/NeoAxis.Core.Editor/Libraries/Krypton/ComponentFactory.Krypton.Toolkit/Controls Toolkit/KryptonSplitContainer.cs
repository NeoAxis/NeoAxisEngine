...
namespace Internal.ComponentFactory.Krypton.Toolkit
{
    ...
    // especially if we have a KSC added as child in parent KSC.
    // you can try to use an intermediate panel with Fill docking as workaround:
    //      KSC panel -> panel with Docking==Fill -> control with anchors.
    //      but this will not to work for KSC in KSC.
                                         ...
                                         ISeparatorSource
    {
        ...
        public Cursor DesignGetHitTest(Point pt)
        {
            ...
            if (_drawSeparator.ClientRectangle.Contains(pt) || _separatorController.IsMoving)
            {
                ...
                if (!Collapsed)
                {
                    ...
                    if (Orientation == Orientation.Vertical)
                        return KryptonCursors.VSplit;
                    else
                        return KryptonCursors.HSplit;
                }
            }
        ...
        //{
        //    //HACK: prevent layout on repaint. WINE bug workaround?
        //    base.OnNeedPaint(sender, new NeedLayoutEventArgs(false));
        //}
        ...
        {
            get { return _splitterPercent; }
            set { _splitterPercent = value; }
        }
    }
}
