...
namespace NeoAxis.Editor
{
	public partial class NewObjectWindow : DockWindow
	{
		...
		public NewObjectWindow()
		{
			...
			WindowTitle = EditorLocalization2.Translate( "NewObjectWindow", WindowTitle );
			EditorLocalization2.TranslateForm( "NewObjectWindow", eUserControl1 );
		...
		static Type GetNewObjectSettingsType( NewObjectSettingsAttribute attrib )
		{
			if( !string.IsNullOrEmpty( attrib.SettingsClassName ) )
			{
				var type = EditorUtility.GetTypeByName( attrib.SettingsClassName );
				if( type == null )
					Log.Warning( $"NewObjectWindow: GetNewObjectSettingsType: Class with name \"{attrib.SettingsClassName}\" is not found." );
				return type;
			}
			else
				return attrib.SettingsClass;
		}
			...
			if( SelectedType != null )
			{
				//add cells by EditorNewObjectSettingsAttribute
				{
					...
					if( ar.Length != 0 || IsFileCreation() )
					{
						...
						if( ar.Length != 0 )
						{
							settingsClass = GetNewObjectSettingsType( (NewObjectSettingsAttribute)ar[ 0 ] );
		...
		bool CreateObject()
		{
			...
			if( !IsFileCreation() )
			{
				//update document
				//!!!!!
				var document = creationData.initDocumentWindow.Document2;
				if( document != null )
				{
					//!!!!только компоненты?
			...
			else
			{
				...
				if( creationData.createdFromContentBrowser != null )
				{
					...
					EditorAPI2.SelectComponentsInMainObjectsWindow( creationData.initDocumentWindow, creationData.createdComponentsOnTopLevel.ToArray() );
				...
				if( creationData.createdComponentsOnTopLevel.Count == 1 )
				{
					...
					if( !component.EditorReadOnlyInHierarchy )
					{
						...
						if( component is FlowGraph || component is CSharpScript )
							EditorAPI2.OpenDocumentWindowForObject( creationData.initDocumentWindow.Document2, component );
					}
