	...
	public static class ResourcesWindowItems
	{
		...
		static void AddBaseItems()
		{
			...
			//Common 3D
			{
				...
				AddItem( new Item( group + @"\Animation Trigger", typeof( AnimationTrigger ) ) );
			...
			{
				{
					...
					list.Add( ("Microparticles In Air", "RenderingEffect_MicroparticlesInAir") );
...

				{
					...
					list.Add( ("Precipitation", "RenderingEffect_Precipitation") );
		...
		internal static void RegisterAssembly( Type[] exportedTypes )
		{
			...
			{
				var attributes = type.GetCustomAttributes<AddToResourcesWindowAttribute>().ToArray();
