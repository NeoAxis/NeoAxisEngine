...
namespace NeoAxis.Editor
{
	public partial class ObjectSettingsHeader_ObjectInfo : ObjectSettingsHeader
	{
		...
		{
			if( ObjectSettingsWindow?.Document2 != null )
		...
		void UpdateButtons()
		{
			...
			bool projectSettings = ObjectSettingsWindow?.Document2?.SpecialMode == "ProjectSettingsUserMode";
		...
		private void buttonTypeSettings_Click( object sender, EventArgs e )
		{
			...
			if( component != null )
			{
				var form = new ComponentTypeSettingsForm( ObjectSettingsWindow.Document2, component );
		...
		private void buttonTypeSettingsDefaultValue_Click( object sender, EventArgs e )
		{
			...
			if( component != null )
			{
				...
				if( EditorMessageBox.ShowQuestion( text, EMessageBoxButtons.YesNo ) == EDialogResult.Yes )
				{
					...
					ObjectSettingsWindow.Document2.UndoSystem.CommitAction( new UndoActionPropertiesChange( undoItem ) );
					ObjectSettingsWindow.Document2.Modified = true;
				}
			}
		}
