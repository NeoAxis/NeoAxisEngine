...
namespace NeoAxis.Editor
{
	public class MeshSettingsCell : SettingsCellProcedureUI
	{
		...
		protected override void OnInit()
		{
			...
			buttonExport = ProcedureForm.CreateButton( EditorLocalization2.Translate( "General", "Export to FBX" ) );
			buttonExport.Click += ButtonExport_Click;

			buttonBuildStructure = ProcedureForm.CreateButton( EditorLocalization2.Translate( "General", "Build Structure" ) );
			buttonBuildStructure.Click += ButtonBuildStructure_Click;

			buttonDeleteStructure = ProcedureForm.CreateButton( EditorLocalization2.Translate( "General", "Delete Structure" ) );
		...
		private void ButtonDeleteStructure_Click( ProcedureUI.Button sender )
		{
			...
			if( EditorMessageBox.ShowQuestion( EditorLocalization2.Translate( "General", "Delete structure?" ), EMessageBoxButtons.YesNo ) == EDialogResult.Yes )
			{
				var property = (Metadata.Property)mesh.MetadataGetMemberBySignature( "property:" + nameof( Mesh.Structure ) );
				var undoAction = new UndoActionPropertiesChange( new UndoActionPropertiesChange.Item( mesh, property, mesh.Structure ) );
