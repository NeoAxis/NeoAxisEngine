...
{
	public partial class FlowGraphEditor : DocumentWindowWithViewport, IFlowGraphEditor
	{
		...
		//!!!!а это не в стиле? что еще туда
		FontComponent nodeFont;
		//!!!!not used, maybe need
		//EngineFont nodeFontValue;
		//Font nodeFontToolTip;
		//Font nodeFontComment;
		...
		//!!!!
		//Second: input flag.
		FlowGraphNode.Representation.Connector referenceCreationSocketFrom;
		...
		FlowGraphNode nodeMove_StartOverNode;
		//!!!!проверять не удалились ли. или запретить удаление. где еще так
		ESet<FlowGraphNode> nodeMove_Nodes;
		Dictionary<FlowGraphNode, Vector2I> nodeMove_StartPositions;

		//reference selection
		FlowGraphNode.Representation.Item mouseOverReference;
		...
		{
			if( nodeFont == null )
				nodeFont = ResourceManager.LoadResource<FontComponent>( @"Base\Fonts\FlowGraphEditor.ttf" );
		...
		//FlowchartNode GetMouseOverNode()
		{
			//!!!!всегда получать или кешировать?
			//!!!!если уже удален? удалить весь список, если кто-то удален

			foreach( FlowGraphNode node in FlowGraph.GetComponents<FlowGraphNode>( true ) )
			{
		...
		//	//if( obj != null )
		//	//{
		//	//	if( obj is FlowchartNode )
		//	//		return true;
		//	//	//!!!!!
		//	//	//if( obj is FlowchartNode.Link )
		//	//	//	return true;
		//	//}
		//	//return false;
		//}
		...
		//!!!!было
		//void EditPinValue( FlowchartNode.Pin pin, Point location )
		//{
		//{
		...
		//	foreach( object obj in objects )
		//	{
		//		FlowchartNode.Link link = obj as FlowchartNode.Link;
		//		if( link != null )
		...
		//	foreach( object obj in objects )
		//	{
		//		FlowchartNode node = obj as FlowchartNode;
		//		if( node != null )
		//		{
		//			foreach( FlowchartNode.Pin input in node.GetSettings().Inputs )
		//			{
		//				foreach( Blueprint.Graph.GetAllLinksFromInputPin_Result result in graph.GetAllLinksFromInputPin( node, input.Name ) )
		//				{
		//					foreach( FlowchartNode.Link link in result.OutputNode.GetAllOutputLinks( result.OutputPinName ) )
		//					{
		...
		//public bool CanCreateReferenceDragDropSetReference( DragDropSetReferenceData dragDropData, Component objectTo, Metadata.Property propertyTo )
		{
			...
			var itemToObject = to.item as FlowGraphNode.Representation.ItemThisObject;
			...
			var itemToProperty = to.item as FlowGraphNode.Representation.ItemProperty;
		...
		public bool CanCreateReference( FlowGraphNode.Representation.Connector c1, FlowGraphNode.Representation.Connector c2 )
		{
			...
			FlowGraphNode.Representation.Connector connectorFrom;
			FlowGraphNode.Representation.Connector connectorTo;
			if( !c1.input )
			{
				connectorFrom = c2;
				connectorTo = c1;
			...
			var itemFrom = connectorFrom.item as FlowGraphNode.Representation.ItemProperty;
			if( itemFrom != null )
			{
				//reference to Component
				var itemToObject = connectorTo.item as FlowGraphNode.Representation.ItemThisObject;
				if( itemToObject != null )
				{
				...
				//reference to property
				var itemToProperty = connectorTo.item as FlowGraphNode.Representation.ItemProperty;
				if( itemToProperty != null )
				{
		...
		//!!!!!было
		//List<FlowchartNode> CloneSelectedNodesAndLinks( bool addOffsetToNodePosition, bool selectClonedNodes )
		//{
		//	List<FlowchartNode> clonedNodes = new List<FlowchartNode>();

		//	Dictionary<FlowchartNode, FlowchartNode> sourceToClonedNodes =
		//		new Dictionary<FlowchartNode, FlowchartNode>();

		//	List<FlowchartNode> sourceNodes = GetSelectedNodes();
		//	foreach( FlowchartNode sourceNode in sourceNodes )
		//	{
		//		FlowchartNode clonedNode = sourceNode.Owner.CreateNode(
		...
		//	foreach( FlowchartNode sourceNode in sourceNodes )
		//	{
		//		FlowchartNode clonedNode = sourceToClonedNodes[ sourceNode ];
		//		foreach( FlowchartNode.Link sourceLink in sourceNode.Links )
		//		{
		//			FlowchartNode sourceInputNode = graph.GetNodeByName( sourceLink.InputNode );
		//			if( sourceInputNode != null )
		//			{
		//				FlowchartNode nodeTo;
		...
		//		foreach( FlowchartNode node in clonedNodes )
		...
		protected override void Viewport_MouseDown( Viewport viewport, EMouseButtons button, ref bool handled )
		{
			...
			if( button == EMouseButtons.Left )
			{
				...
				var mouseOverNode = mouseOverObject as FlowGraphNode;
				...
				if( mouseOverNode != null )
				{
					...
					nodeMove_Nodes = new ESet<FlowGraphNode>();
					...
					{
						var n = selectedObject as FlowGraphNode;
						if( n != null )
						...
						if( c != null )
						{
							if( objectToNodes.TryGetValue( c, out List<FlowGraphNode> nodes ) )
							{
					...
					nodeMove_StartPositions = new Dictionary<FlowGraphNode, Vector2I>();
		...
		protected override void Viewport_MouseUp( Viewport viewport, EMouseButtons button, ref bool handled )
		{
			...
			if( button == EMouseButtons.Left )
			{
				...
				if( allowSelect )
				{
					...
					//select node
					var node = mouseOverObject as FlowGraphNode;
					if( node != null )
					{
				...
				if( referenceCreationSocketFrom != null )
				{
					if( mouseOverSocket != null && CanCreateReference( referenceCreationSocketFrom, mouseOverSocket ) )
					{
						...
						bool isFlow = false;
						{
							var item = referenceCreationSocketFrom.item as FlowGraphNode.Representation.ItemProperty;
							if( item != null )
							{
				...
				if( nodeMove_Activated )
				{
					...
					{
						//cloned
						var action = new UndoActionComponentCreateDelete( Document2, nodeMove_Nodes.ToArray(), true );
						Document2.UndoSystem.CommitAction( action );
						Document2.Modified = true;
		...
		protected override void Viewport_MouseMove( Viewport viewport, Vector2 mouse )//, ref bool handled )
		{
			...
			if( nodeMove_Enabled && !nodeMove_Activated )
			{
				...
				if( Math.Abs( diff.X ) > 2 || Math.Abs( diff.Y ) > 2 )
				{
					...
					{
						FlowGraphNode nodeMove_StartOverNodeSource = nodeMove_StartOverNode;
						ESet<FlowGraphNode> nodeMove_NodesSource = nodeMove_Nodes;
						Dictionary<FlowGraphNode, Vector2I> nodeMove_StartPositionsSource = nodeMove_StartPositions;
						...
						{
							var node = (FlowGraphNode)EditorUtility.CloneComponent( sourceNode );
						...
						//	//if( shiftPressed )
						//	//{
						//	//	List<FlowchartNode> selectedNodes = GetSelectedNodes();
						//	//	if( selectedNodes.Count != 0 )
						//	//	{
						//	//		int nodeMoveStartOverNodeIndex = selectedNodes.IndexOf( nodeMoveStartOverNode );

						//	//		List<FlowchartNode> nodes = CloneSelectedNodesAndLinks( false, true );
						...
						//	//		nodeMoveStartPositions = new Dictionary<FlowchartNode, Vec2I>();
						//	//		foreach( FlowchartNode n in GetSelectedNodes() )
		...
		List<FlowGraphNode> GetNodesByRectangle( Rectangle rectInUnits )
		{
			List<FlowGraphNode> result = new List<FlowGraphNode>();

			foreach( FlowGraphNode node in FlowGraph.GetComponents<FlowGraphNode>( false ) )
			{
				var style = node.GetResultStyle( FlowGraph );
		...
		ESet<FlowGraphNode> SelectByRectangle_GetNodes()
		{
			ESet<FlowGraphNode> result = new ESet<FlowGraphNode>();
			if( selectByRectangle_Activated )
			{
		...
		Dictionary<Component, List<FlowGraphNode>> GetObjectToNodesDictionary()
		{
			var result = new Dictionary<Component, List<FlowGraphNode>>();
			...
			foreach( FlowGraphNode node in FlowGraph.GetComponents<FlowGraphNode>( false ) )
			{
				...
				if( obj != null )
				{
					if( !result.TryGetValue( obj, out List<FlowGraphNode> list ) )
					{
		...
		{
			FlowGraphStyle flowGraphStyle = FlowGraph.Style;
			if( flowGraphStyle == null )
			{
				flowGraphStyle = FlowGraphStyle_Default.Instance;
				//flowchartStyle = (FlowchartStyle_Default)MetadataManager.GetTypeOfNetType( typeof( FlowchartStyle_Default ) ).AutoCreatedInstance;
				//flowchartStyle = ResourceManager.GetProvidedReadOnlyInstanceAsResource<FlowchartStyle>();
			}
		...
		protected override void Viewport_UpdateBeforeOutput( Viewport viewport )
		{
			...
			var referenceSelectionStates = new Dictionary<FlowGraphNode.Representation.Item, RenderSelectionState>();
			{
				if( mouseOverReference != null && CanSelectObjects() )
					referenceSelectionStates[ mouseOverReference ] = RenderSelectionState.CanSelect;
				foreach( var obj in SelectedObjects )
				{
					var item = obj as FlowGraphNode.Representation.Item;
					if( item != null )
						referenceSelectionStates[ item ] = RenderSelectionState.Selected;
				}
			}
			...
			FlowGraphNode.Representation.Item newMouseOverReference = null;
			...
			foreach( FlowGraphNode node in FlowGraph.GetComponents<FlowGraphNode>( false ) )
			{
				...
				//render references
				style.RenderNodeReferences( this, node, objectToNodes, referenceSelectionStates, out FlowGraphNode.Representation.Item outMouseOver );
			...
			//draw nodes
			{
				...
				//foreach( FlowchartNode node in Flowchart.GetComponents<FlowchartNode>( false ) )
				//{
				//	if( node is BlueprintGraphNodeComment )
				//	{
				//		FlowchartNode.Settings settings = node.GetSettings();
				...
				foreach( FlowGraphNode node in FlowGraph.GetComponents<FlowGraphNode>( false ) )
				{
					...
					var selectionStateNode = RenderSelectionState.None;

					if( selectByRectangleNodes.Contains( node ) && CanSelectObjects() )
						selectionStateNode = RenderSelectionState.CanSelect;

					if( IsObjectSelected( node ) )
						selectionStateNode = RenderSelectionState.Selected;
					else
					{
						//select node without assigned controlled object
						if( referenceCreationSocketFrom == null )
					...
					var selectionStateControlledObject = RenderSelectionState.None;
					...
					if( controlledObject != null )
					{
						if( referenceCreationSocketFrom == null )
						{
							if( mouseOverNode != null && mouseOverNode.ControlledObject.Value == controlledObject && CanSelectObjects() )
								selectionStateControlledObject = RenderSelectionState.CanSelect;
						}
						if( IsObjectSelected( controlledObject ) )
							selectionStateControlledObject = RenderSelectionState.Selected;
					}
			...
			if( referenceCreationSocketFrom != null )//!!!!!проверить что не удален
			{
				...
				if( mouseOverSocket != null && CanCreateReference( referenceCreationSocketFrom, mouseOverSocket ) )
					...
					toPositionInUnits = ConvertScreenToUnit( ViewportControl2.Viewport.MousePosition, true );
		...
		public bool CanNewObject( out List<Component> parentsForNewObjects )
		{
			...
			foreach( var obj in SelectedObjects )
			{
				var referenceItem = obj as FlowGraphNode.Representation.Item;
		...
		public void TryNewObject( Metadata.TypeInfo lockType )
		{
			...
			if( parentsForNewObjects.Count == 1 && parentsForNewObjects[ 0 ] == FlowGraph )
			{
				...
				nodeCreationOnGraphPosition = ConvertScreenToUnit( ViewportControl2.Viewport.MousePosition, true ).ToVector2I();
			}
			...
			{
				if( nodeCreationOnGraph && !typeof( FlowGraphNode ).IsAssignableFrom( selectedType.GetNetType() ) )
				{
					window.creationData.createdObjects = new List<object>();

					//create node with object inside of selected type
					var node = FlowGraph.CreateComponent<FlowGraphNode>( -1, false );
		...
		public override bool TryDeleteObjects()
		{
			...
			foreach( var objectToDelete in objectsToDelete )
			{
				...
				var referenceItem = objectToDelete as FlowGraphNode.Representation.ItemProperty;
				if( referenceItem != null )
				{
			...
			if( componentsToDelete.Count != 0 && propertiesChange.Count != 0 )
			{
				...
				multiAction.Actions.Add( new UndoActionComponentCreateDelete( Document2, componentsToDelete, false ) );
				...
				resultAction = new UndoActionComponentCreateDelete( Document2, componentsToDelete, false );
			else if( propertiesChange.Count != 0 )
				resultAction = new UndoActionPropertiesChange( propertiesChange.ToArray() );

			Document2.UndoSystem.CommitAction( resultAction );
		...
		public override bool CanCloneObjects( out List<Component> resultObjectsToClone )
		{
			...
			foreach( var obj in SelectedObjects )
			{
				...
				if( component != null )
				{
					if( objectToNodes.TryGetValue( component, out List<FlowGraphNode> nodes ) )
					{
		...
		private void FlowGraph_DocumentWindow_DragEnter( object sender, DragEventArgs e )
		{
		...
		private void FlowGraph_DocumentWindow_DragOver( object sender, DragEventArgs e )
		{
		...
		private void FlowGraph_DocumentWindow_DragLeave( object sender, EventArgs e )
		{
		...
		private void FlowGraph_DocumentWindow_DragDrop( object sender, DragEventArgs e )
		{
		...
		void DragDropObjectCreate( DragEventArgs e )
		{
				...
				if( dragDropData != null )
				{
					...
					if( memberItem != null )
					{
						...
						if( parentItemComponent != null )
						{
							...
							if( parentComponent.ParentRoot == Document2.ResultComponent )
			...
			if( createComponentType != null || memberFullSignature != "" || createNodeWithComponent != null )
			{
				...
				{
					var context = new FlowGraphSpecialization.DragDropObjectCreateInitNodeContext();
				...
				{
					//CSharpScript
					if( !skipCreation && controlledObject == null && createComponentType != null && MetadataManager.GetTypeOfNetType( typeof( CSharpScript ) ).IsAssignableFrom( createComponentType ) )
					{
						var type = createComponentType as Metadata.ComponentTypeInfo;
						if( type != null && type.BasedOnObject != null )
						{
							var scriptType = type.BasedOnObject as CSharpScript;
							if( scriptType != null )
							{
					...
					//create component
					if( !skipCreation && controlledObject == null && createComponentType != null && createComponentType != MetadataManager.GetTypeOfNetType( typeof( FlowGraphNode ) ) )
					{
		...
		void CalculateDropPosition( FlowGraphNode node )
		{
		...
		void DragDropObjectUpdate()
		{
			if( dragDropObject != null )
			{
				var node = dragDropObject as FlowGraphNode;
				if( node != null )
		...
		public override bool Paste()
		{
			...
			if( data != null )
			{
				...
				for( int n = 0; n < components.Count; n++ )
				{
					...
					{
						var node = cloned as FlowGraphNode;
						if( node != null )
						{
							if( n == 0 )
							{
								CalculateDropPosition( node );
								addToPosition = node.Position - ( (FlowGraphNode)c ).Position;
							}
							else
		...
		protected override object OnGetSelectObjectWhenNoSelectedObjects()
		{
			...
			//Material specific
			var specialization = FlowGraph?.Specialization.Value;
			if( specialization as FlowGraphSpecialization_Shader != null )
			{
