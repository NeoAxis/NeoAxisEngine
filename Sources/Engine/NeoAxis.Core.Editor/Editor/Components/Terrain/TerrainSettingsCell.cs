...
namespace NeoAxis.Editor
{
	public class TerrainSettingsCell : SettingsCellProcedureUI
	{
		...
		{
			buttonUpdate = ProcedureForm.CreateButton( EditorLocalization2.Translate( "Terrain", "Update" ), ProcedureUI.Button.SizeEnum.Long );
			buttonUpdate.Click += ButtonUpdate_Click;

			buttonResizeMasks = ProcedureForm.CreateButton( EditorLocalization2.Translate( "Terrain", "Resize Masks" ), ProcedureUI.Button.SizeEnum.Long );
			...
			buttonImportHeightmap = ProcedureForm.CreateButton( EditorLocalization2.Translate( "Terrain", "Import Heightmap" ), ProcedureUI.Button.SizeEnum.Long );
			buttonImportHeightmap.Click += ButtonImportHeightmap_Click;

			buttonExportHeightmap = ProcedureForm.CreateButton( EditorLocalization2.Translate( "Terrain", "Export Heightmap" ), ProcedureUI.Button.SizeEnum.Long );
		...
		private void ButtonResizeMasks_Click( ProcedureUI.Button sender )
		{
			...
			var text = string.Format( EditorLocalization2.Translate( "Terrain", "Resize masks of selected layers to {0}x{0}?" ), terrain.GetPaintMaskSizeInteger() );
		...
		private void ButtonExportHeightmap_Click( ProcedureUI.Button obj )
		{
			...
			if( !terrain.SaveHeightmapBuffer( fileName, out var error ) )
				...
				ScreenNotifications2.Show( EditorLocalization2.Translate( "Terrain", "The image was created successfully." ) );
		}
	}
}
