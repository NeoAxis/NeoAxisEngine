...
{
	public class TerrainEditingMode : SceneEditorWorkareaMode
	{
		...
		public SceneEditor DocumentWindow2
		{
			get { return (SceneEditor)DocumentWindow; }
		}
		...
		protected override bool OnKeyDown( Viewport viewport, KeyEvent e )
		{
			...
			{
				DocumentWindow2.ResetWorkareaMode();
				return true;
			}
		...
		protected override bool OnMouseDown( Viewport viewport, EMouseButtons button )
		{
			if( button == EMouseButtons.Left )
			{
				...
				{
					var layer = DocumentWindow2.TerrainPaintLayersGetSelected();
					...
					if( IsCurrentPaintTool() && layer != null )
					{
						if( layer.Mask.Value != null && layer.Mask.Value.Length != 0 )
						{
							...
							{
								EditorMessageBox.ShowWarning( EditorLocalization2.Translate( "Terrain", "Unable to paint to selected layer because Mask size of the layer and MaskSize of the terrain are not equal." ) );
								return true;
							}
						}
					}
