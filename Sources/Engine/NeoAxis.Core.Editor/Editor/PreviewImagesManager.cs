...
namespace NeoAxis.Editor
{
	...
	{
		//!!!!config
		const int imageSizeRender = 1024;//512;//256
		...
		class Processor
		{
			...
			{
				var attribs = (PreviewImageAttribute[])obj.GetType().GetCustomAttributes( typeof( PreviewImageAttribute ), true );
				if( attribs.Length != 0 )
				{
			...
			public void StartTask( ImageToProcessItem imageToProcess )
			{
				...
				try
				{
					...
					if( generator != null )
					{
						...
						demandedFrame = textureRead.Result.GetNativeObject( true ).Read( imageData, 0, 0 );
					}
					else
						ClearTask();
				}
			...
			public void Update()
			{
				if( Working )
				{
					try
					{
						if( RenderingSystem.LastFrameNumber >= demandedFrame && generator != null )
						{
							//task is done
							...
							//make result bitmap
							var bitmapResult = Win32Utility.ResizeImage( bitmap2, imageSizeResult, imageSizeResult );
		...
		{
			var types = new string[] { "Material", "Image", "Mesh", "Import 3D", "Sky", "Object In Space", "Surface" };
		...
		static LoadedPreviewItem GetOrLoadPreview( string virtualFileName, out bool cacheNotExists )
		{
			...
			if( item == null || item.needReload )
			{
				...
				if( File.Exists( cacheFileName ) )
				{
					...
					int treeSize;
					if( EditorAPI2.DPIScale >= 2.0 )
						treeSize = 32;
					else if( EditorAPI2.DPIScale >= 1.75 )
						treeSize = 28;
					else if( EditorAPI2.DPIScale >= 1.5 )
						treeSize = 24;
					else
						treeSize = 16;
					item.smallImageForTreeView = Win32Utility.ResizeImage( item.originalImage, treeSize, treeSize );
		...
		public static Image GetImageForResource( string realFileName, bool forTreeView )
		{
			try
			{
				...
				{
					var virtualFileName = VirtualPathUtility.GetVirtualPathByReal( realFileName, true );
		...
		//!!!!specific
		public static Image GetImageForPaintLayer( PaintLayer layer )//public static Image GetImage( Component component )
		{
			if( layer != null )
			{
		...
		public static void NeedRegenerateFile( string virtualFileName )
		{
			...
			if( item != null )
			{
				//!!!!может не сразу обновл€ть, с задержкой
