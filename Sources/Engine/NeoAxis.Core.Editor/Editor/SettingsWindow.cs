	...
	public partial class SettingsWindow : DockWindow
	{
		...
		public SettingsWindow()
		{
			...
			if( !WinFormsUtility.IsDesignerHosted( this ) )
				settingsDisplayHierarchyOfObjectsInSettingsWindow = ProjectSettings.Get.General.DisplayHierarchyOfObjectsInSettingsWindow;
		...
		private void timer1_Tick( object sender, EventArgs e )
		{
			...
			if( selectedPanel != null )
			{
				var enable = GetEnabledForSelectedPanel();
				if( selectedPanel.layoutPanel.Enabled != enable )
				{
					KryptonWinFormsUtility.EditorFormStartTemporaryLockUpdate();
					selectedPanel.layoutPanel.Enabled = enable;
				}
			}

			//check for DisplayHierarchyOfObjectsInSettingsWindow update
			if( settingsDisplayHierarchyOfObjectsInSettingsWindow != ProjectSettings.Get.General.DisplayHierarchyOfObjectsInSettingsWindow.Value )
			{
				settingsDisplayHierarchyOfObjectsInSettingsWindow = ProjectSettings.Get.General.DisplayHierarchyOfObjectsInSettingsWindow.Value;
				...
				foreach( object obj in objs )
				{
					...
					if( c != null )
					{
						if( c.ParentRoot.HierarchyController != null )
						{
							...
							if( ins != null )
							{
								if( EditorAPI2.GetDocumentByResource( ins ) == null )
									enable = false;
							}
						}
