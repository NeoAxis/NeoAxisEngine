...
namespace NeoAxis.Editor
{
	...
	public partial class BackstageMenu : BackstageAppMenu
	{
		...
		{
			return EditorLocalization2.Translate( "Backstage", text );
		}
		...
		private void BackstageMenu_Load( object sender, EventArgs e )
		{
			...
			//translate
			{
				...
				EditorLocalization2.TranslateForm( "Backstage", kryptonPageInfo );
				EditorLocalization2.TranslateForm( "Backstage", kryptonPageNew );
				EditorLocalization2.TranslateForm( "Backstage", kryptonPageBuild );
		...
		void CreationFunction( object parameter )
		{
			...
			try
			{
				...
				ScreenNotifications2.Show( Translate( "The project was created successfully." ) );
			}
			catch( Exception ex )
			{
				EditorMessageBox.ShowWarning( ex.Message );
