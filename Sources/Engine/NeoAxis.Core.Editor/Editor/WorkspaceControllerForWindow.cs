	...
	class WorkspaceControllerForWindow : WorkspaceController
	{
		...
		internal class WindowConfig
		{
			...
			internal static WindowConfig FromComponent( Component comp )
			{
				...
				{
					OpenAsSettings = EditorAPI2.GetDocumentWindowClass( comp ) == typeof( ObjectSettingsWindow ), //TODO: refactor
		...
		public override void AddDockWindow( DockWindow window, bool floatingWindow, bool select )
		{
			...
			if( documentWindow.Document2 == null )
				throw new ArgumentNullException( nameof( documentWindow.Document2 ) );

			if( documentWindow.Document2 != WorkspaceWindow.Document )
				throw new ArgumentException( nameof( documentWindow.Document2 ) + " should be equal to the " + nameof( WorkspaceWindow.Document ) );
		...
		protected override void OnDockWindowLoading( DockWindow window, DockPageLoadingEventArgs e )
		{
			if( window == null )
			{
				...
				window = EditorAPI2.CreateWindowImpl( WorkspaceWindow.Document2, config.ObjectPath, config.OpenAsSettings );
		...
		protected override void OnDockWindowCloseRequest( DockWindow window, out bool cancel )
		{
			...
			if( documentWindow != null && documentWindow.ObjectOfWindow == documentWindow.Document2.ResultObject && !documentWindow.OpenAsSettings )
			{
				cancel = true;
				return;
			}
