...
namespace NeoAxis.Editor
{
	...
	public abstract class HCItemProperty : HCItemMember
	{
		...
		public override void UpdateControl()
		{
			...
			{
				control.LabelNameSetText( DisplayName );
			...
			if( control.ButtonDefaultValue != null )
			{
				...
				if( buttonDefaultValueCurrentEnabled != canReset && control.ButtonDefaultValue.Visible )
				{
					...
					if( EditorAPI2.DarkTheme )
						control.ButtonDefaultValue.Values.Image = canReset ? EditorResourcesCache.GetImage( EditorAPI2.DPIScale >= 2.0 ? "DefaultValueCircle_Big_Dark" : "DefaultValueCircle3_Dark" ) : null;
					else
						control.ButtonDefaultValue.Values.Image = canReset ? EditorResourcesCache.GetImage( EditorAPI2.DPIScale >= 2.0 ? "DefaultValueCircle_Big" : "DefaultValueCircle3" ) : null;
				}
			}
		}
			...
			if( control.ButtonReference != null )
			{
				...
				if( isReferenceType )
				{
					if( buttonReferenceCurrentEnabled != referenceSpecified && control.ButtonReference.Visible )
					{
						...
						if( EditorAPI2.DarkTheme )
							control.ButtonReference.Values.Image = buttonReferenceCurrentEnabled.Value ? EditorResourcesCache.GetImage( EditorAPI2.DPIScale >= 2.0 ? "ReferenceEnabled2_Dark" : "ReferenceEnabled2_10_Dark" ) : null;
						else
							control.ButtonReference.Values.Image = buttonReferenceCurrentEnabled.Value ? EditorResourcesCache.GetImage( EditorAPI2.DPIScale >= 2.0 ? "ReferenceEnabled2" : "ReferenceEnabled2_10" ) : null;
		...
		void UpdateExpandButton()
		{
			...
			if( control.ButtonExpand != null )
			{
				...
				if( expandButtonImageExpanded != Expanded && control.ButtonExpand.Visible )
				{
					...
					{
						if( Expanded )
							control.ButtonExpand.Values.Image = EditorResourcesCache.GetImage( EditorAPI2.DPIScale >= 1.5 ? "Minus_Big_Dark" : "Minus_small_Dark" );
						else
							control.ButtonExpand.Values.Image = EditorResourcesCache.GetImage( EditorAPI2.DPIScale >= 1.5 ? "Plus_Big_Dark" : "Plus_small3_Dark" );
					}
					else
					{
						if( Expanded )
							control.ButtonExpand.Values.Image = EditorResourcesCache.GetImage( EditorAPI2.DPIScale >= 1.5 ? "Minus_Big" : "Minus_small" );
						else
							control.ButtonExpand.Values.Image = EditorResourcesCache.GetImage( EditorAPI2.DPIScale >= 1.5 ? "Plus_Big" : "Plus_small3" );
					}
				}
			}
		}
		...
		public void AddUndoActionWithSavedValuesToRestore()
		{
			...
			{
				DocumentInstance document = Owner.DocumentWindow.Document2;
				if( document != null )
				{
		...
		{
			EditorAPI2.OpenSetReferenceWindow( Owner.DocumentWindow, GetControlledComponents(), ControlledObjects, property, indexers );
		...
		private void ButtonReference_MouseMove( object sender, MouseEventArgs e )
		{
			...
			if( buttonReferenceMouseLeftDownCanDragDrop )
			{
				...
				if( !button.ClientRectangle.Contains( button.PointToClient( Control.MousePosition ) ) )
				{
					...
					data.document = Owner.DocumentWindow.Document2;
					...
					var item = new KryptonContextMenuItem( Translate( "Open By Reference" ), null, delegate ( object s, EventArgs e2 )
					{
						if( referenceSpecifiedCached )
						{
							...
							if( values != null )
							{
								...
								if( unrefValue != null )
								{
									var document = EditorAPI2.GetDocumentByObject( unrefValue );
									if( EditorAPI2.IsDocumentObjectSupport( unrefValue ) )
										EditorAPI2.OpenDocumentWindowForObject( document, unrefValue );
									else
									{
			...
			if( unrefValue != null )
			{
				...
				if( IsOneDimensionArray( unrefValueType.GetNetType() ) )
				{
					...
					if( WasExpanded )//!!!!может проверять уже когда точно понятно что есть вложенные
					{
						...
						//Item properties
						if( length <= ProjectSettings.Get.General.PropertiesMaxCountCollectionItemsToDisplay )
						{
				...
				if( IsListType( unrefValueType.GetNetType() ) )
				{
					...
					if( WasExpanded )//!!!!может проверять уже когда точно понятно что есть вложенные
					{
						...
						//Item properties
						if( count <= ProjectSettings.Get.General.PropertiesMaxCountCollectionItemsToDisplay )
						{
		...
		bool DragDropSetReferenceFromContentBrowser( DragEventArgs e, bool checkOnly )
		{
			...
			if( dragDropData != null )
			{
				...
				{
					if( !checkOnly )
						EditorUtility.SetPropertyReference( Owner.DocumentWindow.Document2, ControlledObjects, property, indexers, referenceValues );
					//if( !checkOnly )
		...
		private void ButtonType_Click( object sender, EventArgs e )
		{
			...
			initData.DocumentWindow = Owner.DocumentWindow;
			initData.DemandedTypes = new Metadata.TypeInfo[] { property.TypeUnreferenced };
			initData.CanSelectNull = true;
