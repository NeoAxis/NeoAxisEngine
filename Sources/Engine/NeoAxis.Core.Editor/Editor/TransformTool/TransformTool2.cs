...
{
	/// <summary>
	/// Config for <see cref="TransformToolClass"/>.
	/// </summary>
	...
	/// </summary>
	public partial class TransformToolClass : ITransformTool
	{
		EngineViewportControl viewportControl;

		List<TransformToolObject> objects = new List<TransformToolObject>();
		TransformToolMode mode;
		TransformToolCoordinateSystemMode coordinateSystemMode = TransformToolCoordinateSystemMode.World;
		...
		/// <summary>
		/// Defines the position in space of an object for <see cref="TransformToolClass"/>.
		/// </summary>
		...
		public TransformToolClass( EngineViewportControl viewportControl )
		{
			...
			try
			{
				if( moveCursor == null )
				{
					...
					if( hCursor != IntPtr.Zero )
						...
						moveCursor = new Cursor( new MemoryStream( NeoAxis.Editor.Properties.Resources.MoveCursor ) );
				}
				...
				if( rotateCursor == null )
				{
					...
					if( hCursor != IntPtr.Zero )
						...
						rotateCursor = new Cursor( new MemoryStream( NeoAxis.Editor.Properties.Resources.RotateCursor ) );
				}
				...
				if( scaleCursor == null )
				{
					...
					if( hCursor != IntPtr.Zero )
						...
						scaleCursor = new Cursor( new MemoryStream( NeoAxis.Editor.Properties.Resources.ScaleCursor ) );
				}
			}
			catch { }
		}
		...
		{
			internal TransformToolClass owner;
			...
			public TransformToolClass Owner
			{
				get { return owner; }
			}
			...
			{
				//!!!!как-то странно тени рисуютс€, вроде как неправильно
				//!!!!!!второй вариант рисование теней: с помощью постэффекта. сначала всЄ в текстуру, потом еЄ обработать
			...
			protected void AddPolygonalChain( Vector3[] points, double radius, double drawShadowsFactor )
			{
				...
				var transform = Matrix4.FromTranslate( center );
				DebugGeometry.AddTriangles( positions, indices, ref transform, false, true );
		...
		public TransformToolCoordinateSystemMode CoordinateSystemMode
		{
		...
		//!!!!!!есть Escape, видать, не переопредел€емый
		...
		public delegate void ChangeMofidyStateDelegate( TransformToolClass sender );
		...
		void OnModifyBegin()
		{
			...
			TransformToolUtility.PerformAllInstances_ModifyBegin( this );
		...
		void OnModifyCommit()
		{
			...
			TransformToolUtility.PerformAllInstances_ModifyCommit( this );
		...
		void OnModifyCancel()
		{
			...
			TransformToolUtility.PerformAllInstances_ModifyCancel( this );
