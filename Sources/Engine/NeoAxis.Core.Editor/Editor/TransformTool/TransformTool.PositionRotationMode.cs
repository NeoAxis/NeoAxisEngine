...
{
	public partial class TransformToolClass
	{
		class PositionRotationModeClass : ModeClass
		{
			...
			{
				if( Owner.CoordinateSystemMode == TransformToolCoordinateSystemMode.Local )
				{
			...
			Axes GetAxesByMousePosition()
			{
				...
				bool xPlus;
				{
					...
					if( !Owner.SceneMode2D || Owner.CoordinateSystemMode == TransformToolCoordinateSystemMode.Local )
			...
			void Rotation_OnMouseMove()
			{
				if( modify_Activated && !owner.viewportControl.Viewport.MouseRelativeMode && !modifyIsPosition )
				{
					...
					Quaternion startRotation = GetStartObjectsRotation();
					double scaleFactor = ProjectSettings.Get.SceneEditor.TransformToolRotationSensitivity / 150;
			...
			void Position_Render()
			{
				...
				ColorValue shadowColor = new ColorValue( 0, 0, 0, ProjectSettings.Get.SceneEditor.TransformToolShadowIntensity );
				...
				//Arrows
				{
					...
					for( int nDrawStep = 0; nDrawStep < 5; nDrawStep++ )
					{
						...
						if( drawShadows && ProjectSettings.Get.SceneEditor.TransformToolShadowIntensity == 0 )
							continue;
