...
namespace NeoAxis.Editor
{
	class HCItemProjectShortcuts : HCItemProperty
	{
		...
		ProjectSettingsPage_Shortcuts.ShortcutSettingsClass.ActionItem selectedActionItem;
		...
		{
			return EditorLocalization2.Translate( "ProjectShortcuts", text );
		}
		...
		class ProxyObject
		{
			...
			ProjectSettingsPage_Shortcuts.ShortcutSettingsClass.ActionItem obj;

			//

			static void Convert( ProjectSettingsPage_Shortcuts.Keys2 from3, out EKeys key, out bool shift, out bool control, out bool alt )
			{
				var from = (Keys)from3;
			...
			static void Convert( EKeys key, bool shift, bool control, bool alt, out ProjectSettingsPage_Shortcuts.Keys2 to )
			{
				to = (ProjectSettingsPage_Shortcuts.Keys2)key;
				if( shift )
					to |= ProjectSettingsPage_Shortcuts.Keys2.Shift;
				if( control )
					to |= ProjectSettingsPage_Shortcuts.Keys2.Control;
				if( alt )
					to |= ProjectSettingsPage_Shortcuts.Keys2.Alt;
			}
			...
			public EKeys Shortcut1Key
			{
				...
				set
				{
					...
					if( owner.Owner?.DocumentWindow?.Document2 != null )
						owner.Owner.DocumentWindow.Document2.Modified = true;
				}
			}
			...
			public ProxyObject( HCItemProjectShortcuts owner, ProjectSettingsPage_Shortcuts.ShortcutSettingsClass.ActionItem obj )
			{
				this.owner = owner;
				this.obj = obj;
			}
		...
		internal/*obfuscator*/ void KryptonButtonReset_Click( object sender, EventArgs e )
		{
			...
			if( Owner?.DocumentWindow?.Document2 != null )
				Owner.DocumentWindow.Document2.Modified = true;
		}

		ProjectSettingsPage_Shortcuts.ShortcutSettingsClass GetSettings()
		{
			var control = (HCItemProjectShortcutsForm)CreatedControlInsidePropertyItemControl;
			...
			if( values == null || values.Length != 1 )
				return null;
			return values[ 0 ] as ProjectSettingsPage_Shortcuts.ShortcutSettingsClass;
		}
		...
		void UpdateProperties()
		{
			...
			ProjectSettingsPage_Shortcuts.ShortcutSettingsClass.ActionItem toSelect = null;
