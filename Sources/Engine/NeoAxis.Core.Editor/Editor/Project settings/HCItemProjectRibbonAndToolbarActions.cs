...
namespace NeoAxis.Editor
{
	class HCItemProjectRibbonAndToolbarActions : HCItemProperty
	{
		...
		{
			return EditorLocalization2.Translate( "ProjectSettingsRibbonAndToolbarActions", text );
		}
		...
		internal/*obfuscator*/ void ContentBrowserProject_KeyDownOverride( ContentBrowser browser, object sender, KeyEventArgs e, ref bool handled )
		{
			//Delete
			{
				var shortcuts = EditorAPI2.GetActionShortcuts( "Delete" );
				if( shortcuts != null )
				{
		...
		internal/*obfuscator*/ void ContentBrowserProject_ShowContextMenuEvent( ContentBrowser sender, ContentBrowser.Item contentItem, List<KryptonContextMenuItemBase> items )
		{
			...
			//Delete
			{
				var item = new KryptonContextMenuItem( EditorLocalization2.Translate( "General", "Delete" ), EditorResourcesCache.Delete,
					delegate ( object s, EventArgs e2 )
					{
						TryDeleteObjects();
					} );
			...
			//Rename
			{
				var item = new KryptonContextMenuItem( EditorLocalization2.Translate( "General", "Rename" ), null,
					delegate ( object s, EventArgs e2 )
					{
						TryRename();
					} );
		...
		public void TryDeleteObjects()
		{
			...
			if( itemsToDelete.Count == 1 )
				text = string.Format( EditorLocalization2.Translate( "General", "Delete \'{0}\'?" ), itemsToDelete[ 0 ].ToString() );
			else
				text = EditorLocalization2.Translate( "General", "Delete selected objects?" );
		...
		public void TryRename()
		{
			...
			if( groupItem != null )
			{
				var form = new OKCancelTextBoxForm( EditorLocalization2.TranslateLabel( "General", "Name:" ), groupItem.Name, EditorLocalization2.Translate( "General", "Rename" ),
			...
			if( tabItem != null )
			{
				var form = new OKCancelTextBoxForm( EditorLocalization2.TranslateLabel( "General", "Name:" ), tabItem.Name, EditorLocalization2.Translate( "General", "Rename" ),
					delegate ( string text, ref string error )
					{
						if( string.IsNullOrEmpty( text.Trim() ) )
							return false;
