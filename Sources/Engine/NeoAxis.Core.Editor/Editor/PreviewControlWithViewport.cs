...
namespace NeoAxis.Editor
{
	...
	public partial class PreviewControlWithViewport : PreviewControl, IPreviewControlWithViewport
	{
		//scene
		Scene scene;
		...
		{
			if( ViewportControl2.Viewport.MouseRelativeMode )
			{
				//!!!sens в настройки
		...
		protected virtual void Viewport_Tick( Viewport sender, float delta )
		{
			if( viewportControl != null )
				viewportControl.AutomaticUpdateFPS = (float)ProjectSettings.Get.General.MaxFramesPerSecondPreview;
		}
		...
		protected virtual void Viewport_UpdateBeforeOutput( Viewport viewport )
		{
			...
			//if( scene != null )
			//{
			//	foreach( var obj in scene.GetComponents<ObjectInSpace>( false, true ) )
			//	{
