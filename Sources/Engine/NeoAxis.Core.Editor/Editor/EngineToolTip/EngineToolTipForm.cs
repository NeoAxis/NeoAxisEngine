	...
	public partial class EngineToolTipForm : Form//EngineForm
	{
		...
		private void EngineToolTipForm_Load( object sender, EventArgs e )
		{
			...
			using( var g = CreateGraphics() )
			{
				...
				var size = new Vector2I( (int)( textSize.Width + EditorAPI2.DPIScale * 7.0f ), (int)( textSize.Height + EditorAPI2.DPIScale * 7.0f ) );

				//fix Location when outsize the screen
				var screenSize = SystemSettings.AllDisplaysBounds.RightBottom;
				var location = Location;
				if( Location.X + size.X + (int)( EditorAPI2.DPIScale * 2.0f ) > screenSize.X )
					location.X = screenSize.X - size.X - (int)( EditorAPI2.DPIScale * 2.0f );
				if( Location.Y + size.Y + (int)( EditorAPI2.DPIScale * 2.0f ) > screenSize.Y )
					location.Y = screenSize.Y - size.Y - (int)( EditorAPI2.DPIScale * 2.0f );
				if( location != Location )
					Location = location;

				Size = new Size( size.X, size.Y );
				labelBody.Location = new Point( (int)( EditorAPI2.DPIScale * 2.0f ), 0 );
