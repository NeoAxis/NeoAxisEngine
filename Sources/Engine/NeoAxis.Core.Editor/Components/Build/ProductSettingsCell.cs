...
namespace NeoAxis.Editor
{
	public class ProductSettingsCell : SettingsCellProcedureUI
	{
		...
		{
			buttonBuild = ProcedureForm.CreateButton( EditorLocalization2.Translate( "General", "Build" ) );
			buttonBuild.Click += ButtonBuild_Click;

			buttonBuildAndRun = ProcedureForm.CreateButton( EditorLocalization2.Translate( "General", "Build and Run" ), ProcedureUI.Button.SizeEnum.Long );
		...
		protected override void OnUpdate()
		{
			...
			if( product != null )
			{
				...
				if( product as Product_Store != null )
					buttonBuildAndRun.Text = EditorLocalization2.Translate( "General", "Build and Upload" );
			}
			else
			{
