...
namespace NeoAxis.Editor
{
	...
	public class VehicleEditorAssemblyRegistration : AssemblyRegistration
	{
		...
		private void Product_Store_CreateScreenshots( Product_Store sender, string[] files, ZipArchive archive, bool additional, string productFilesPathPrefix, ref int imageCounter, ref bool handled )
		{
			var virtualFileName = "";
				...
				if( vehicleType != null )
				{
					if( !additional )
					{
						...
						using( var stream = File.OpenWrite( fullPath ) )
							generator.Generate( vehicle, stream );
					...
					else
					{
						...
						//!!!!one image is disabled

						for( int nImage = 0; nImage < 4; nImage++ ) //for( int nImage = 0; nImage < 5; nImage++ )
						{
							//in the scene
							if( nImage <= 2 ) //if( nImage <= 3 )
							{
								var generator = new Product_Store.ImageGenerator();
								...
								{
									var fullPath = productFilesPathPrefix + $"_Image{imageCounter}.png";
									using( var stream = File.OpenWrite( fullPath ) )
										generator.Generate( null/*vehicle*/, stream );
									imageCounter++;
