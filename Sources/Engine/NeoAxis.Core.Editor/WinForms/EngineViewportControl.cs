...
namespace NeoAxis.Editor
{
	...
	/// </summary>
	public partial class EngineViewportControl : EUserControl, IEngineViewportControl
	{
		...
		[Browsable( false )]
		public TransformToolClass TransformTool { get; set; }
		...
		[Browsable( false )]
		public bool DisableRecreationRenderWindow
		{
			get { return disableRecreationRenderWindow; }
			set { disableRecreationRenderWindow = value; }
		}
		...
		//static Bitmap GetSplashLogoImage( ProjectSettingsPage_General.EngineSplashScreenStyleEnum style )
		//{
		...
		void UpdateKeysAndMouseButtons()
		{
			...
			//keys
			foreach( EKeys eKey in NeoAxis.Viewport.AllKeys )
			{
				if( this.Viewport.IsKeyPressed( eKey ) )
				{
					...
					if( Enum.IsDefined( typeof( EKeys ), keyCode ) && GetKeyState( keyCode ) >= 0 )
					{
						KeyEvent keyEvent = new KeyEvent( eKey );
						bool handled = false;
						this.Viewport.PerformKeyUp( keyEvent, ref handled );
					}
				}
			}
		}
