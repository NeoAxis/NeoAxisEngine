...
namespace ComponentFactory.Krypton.Toolkit
{
    ...
    public abstract class VisualForm : Form, IKryptonDebug
    {
        ...
        // shadows v1 have a problem with deactivating the application when the main form is activated.
        // for example, when you finish drag drop of floating window, the main window and other windows 
        // receive the WM_ACTIVATEAPP (False) event. from NativeMethods.ShowWindow(Handle, NativeMethods.SW_HIDE);

        // the issue disappears if you do not change the owner of the main window in VisualFormShadow ctor
        // and and if do not call TargetForm.Activate(); in VisualFormShadow.OnTargetFormResize

        private VisualFormShadow _shadowForm;
        private VisualFormShadowType _shadowType = VisualFormShadowType.Default;
        private VisualFormShadowType _currentShadowType = VisualFormShadowType.None;
        ...
        [EditorBrowsable(EditorBrowsableState.Never)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public VisualFormShadowType ShadowType {
            get { return _shadowType; }
            set {
                if (_shadowType == value)
                    return;
                _shadowType = value;
                UpdateShadowType(value);
            }
        }
        ...
        {
            UpdateShadowState(ShadowType);
        }

        private void UpdateShadowState(VisualFormShadowType newShadowType)
        {
            if (DesignMode || !IsHandleCreated || IsDisposed)
                return;

            if (newShadowType == VisualFormShadowType.Default)
                newShadowType = VisualFormShadowType.DropShadow;

            if (!IsCustomHeader)
                newShadowType = VisualFormShadowType.None;

            if (_currentShadowType == newShadowType)
                return;

            ReleaseFormShadow();
            if (newShadowType != VisualFormShadowType.None)
                InitFormShadow(newShadowType);

            _currentShadowType = newShadowType;
        }

        private void InitFormShadow(VisualFormShadowType shadowType)
        {
            if (_shadowForm == null)
            {
                switch (shadowType)
                {
                    case VisualFormShadowType.Flat:
                        _shadowForm = new VisualFlatDropShadow(this);
                        break;

                    case VisualFormShadowType.DropShadow:
                        _shadowForm = new VisualRealisticDropShadow(this);
                        break;

                    case VisualFormShadowType.SystemAeroShadow:
                        _shadowForm = new VisualAeroDropShadow(this);
                        break;
                }
            }

            _shadowForm.AllowResizeViaShadows = this.IsModernForm &&
                this.FormBorderStyle == FormBorderStyle.Sizable;
        }

        private void ReleaseShadow()
        {
            UpdateShadowState(VisualFormShadowType.None);
        }
        ...
        private ImageCacheManager _borderImageCacheManager;
        ...
        private void CreateFormShadow()
        {
            if (_borderImageCacheManager == null)
                _borderImageCacheManager = new ImageCacheManager();

            if (_dropShadowManager == null)
            {
                _dropShadowManager = new DropShadowManager(this);
                UpdateShadowColor();
            }
        }
        ...
        private void UpdateShadowColor()
        {
            if (_dropShadowManager != null)
            {
                //TODO: implement _palette.GetShadowColor1(...)

                var color = _palette.GetBorderColor1(PaletteBorderStyle.FormMain, ViewManager.Root.State);

                if( KryptonDarkThemeUtility.DarkTheme )
                    color = Color.FromArgb( 30, 30, 30 );

                _dropShadowManager.ImageCache = _borderImageCacheManager.GetCached(color);
            }
        }
