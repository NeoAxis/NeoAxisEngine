...
namespace CommandLineTools
{
	static partial class EditorCommandLineTools
	{
		public static bool Process()
		{
			...
			{
				var result = PrepareProductAssets.Process();
		...
		static class PrepareProductAssets
		{
			...
			static void CopyFilesToPackageFolder( string pathsParameter )// string selectedAssets, string excludedAssets )
			{
				...
				{
					string sourceFolder = Path.Combine( ProjectFolder, "Binaries", "NeoAxis.Internal" );
					string destFolder = Path.Combine( DestinationFolder, "Binaries", "NeoAxis.Internal" );
				...
				//create Project.zip
				{
					...
					paths.Add( Path.Combine( DestinationFolder, @"Binaries\NeoAxis.Internal\NeoAxis.DefaultSettings.config" ) );
			...
			public static bool Process()
			{
				Console.WriteLine( "CommandLineTools: PrepareProductAssets: Preparing Project.zip..." );

				var exeFullPath = Environment.GetCommandLineArgs()[ 0 ];

				//!!!!
				//{
				//	Console.WriteLine( "EXE FULL PATH:" );
				//	Console.WriteLine( exeFullPath );
				//	Console.WriteLine( "EXE FULL PATH END" );
				//}

				var projectFullPath = "";
				{
					var index = exeFullPath.IndexOf( "\\Binaries\\NeoAxis.Internal" );
					if( index != -1 )
						projectFullPath = exeFullPath.Substring( 0, index );
				}

				//!!!!
				{
					Console.WriteLine( "PROJECT FULL PATH:" );
					Console.WriteLine( projectFullPath );
					Console.WriteLine( "PROJECT FULL PATH END" );
				}

				if( string.IsNullOrEmpty( projectFullPath ) )
				{
					Console.WriteLine( "PrepareProductAssets: Project file path is not extracted from the command line parameters." );
					return false;
				}
				...
				//	Console.WriteLine( "ASSEMBLY LOCATION END" );
				//}
				...
				//!!!!
				{
					...
					Console.WriteLine( "COMMAND LINE END" );
				}

				//!!!!
				{
					Console.WriteLine( "Arguments START:" );
					foreach( var a in SystemSettings.CommandLineParameters )
						Console.WriteLine( a.Key + "=" + a.Value );
					Console.WriteLine( "END" );
				}

				//if( !SystemSettings.CommandLineParameters.TryGetValue( "-projectPath", out var projectPath ) )
				//{
				//	Console.WriteLine( "CommandLineTools: -projectPath is not specified." );
				//	return false;
				//}

				if( !SystemSettings.CommandLineParameters.TryGetValue( "-productFile", out var productFile ) )
				{
					Console.WriteLine( "CommandLineTools: -productFile is not specified." );
					return false;
				}
				...
				{
					Console.WriteLine/*Log.Warning*/( "PrepareProductAssets: -destinationFolder is not specified." );
					return false;
				}

				if( !SystemSettings.CommandLineParameters.TryGetValue( "-platform", out var platform ) )
				{
					Console.WriteLine( "PrepareProductAssets: -platform is not specified." );
					return false;
				}
				if( !Enum.TryParse( platform, out Platform ) )
				{
					Console.WriteLine( "PrepareProductAssets: Unable to parse platform parameter value \'{0}\'.", platform );
					return false;
				}

				if( !Path.IsPathRooted( projectFullPath ) )
				{
					Console.WriteLine( "PrepareProductAssets: -projectPath the path is not rooted." );
					return false;
				}

				//if( !Path.IsPathRooted( productFullPath ) )
				//{
				//	Log.Warning( "PrepareProductAssets: -prepareProductAssets the path is not rooted." );
				//	return;
				//}

				ProjectFolder = projectFullPath;
				AssetsFolder = Path.Combine( ProjectFolder, "Assets" );
				var productFullPath = Path.Combine( AssetsFolder, productFile );
				...
				{
					Console.WriteLine( $"Unable to load \'{productFullPath}\'. " + error );
					return false;
				}
...
again:;
			...
			static bool ProcessVcxproj( string destFile, string baseProjectFileName, out string error, out bool changed )
			{
				error = "";
				changed = false;
				...
				XmlNode itemGroupNode = null;
				{
					var list = xmldoc.SelectNodes( "//df:ClCompile", mgr );
					foreach( XmlNode node in list )
					{
						var attr = node.Attributes[ "Include" ];
						if( attr != null )
				...
				foreach( var fileName in toInclude )
				{
					var node = xmldoc.CreateNode( XmlNodeType.Element, "ClCompile", null );
					var includeAttribute = xmldoc.CreateAttribute( "Include" );
					includeAttribute.Value = fileName;
					node.Attributes.Append( includeAttribute );
					...
					{
						var node2 = xmldoc.CreateNode( XmlNodeType.Element, "CompileAs", null );
						node2.InnerText = "CompileAsC";
				...
				//!!!!
				//foreach( var q in toInclude )
				//	Console.WriteLine( q );

				//!!!!
				//Console.WriteLine( "test" );
				//Console.WriteLine( destFile );
				//Console.WriteLine( baseProjectFileName );
				//Console.WriteLine( "end" );
			...
			public static bool Process( string destFile, string baseProjectFileName, out string error, out bool changed )
			{
				...
				var destExtension = Path.GetExtension( destFile ).ToLower();
				...
				{
					if( !ProcessVcxproj( destFile, baseProjectFileName, out error, out changed ) )
						return false;
					return true;
				}

				error = string.Format( "File type \"{0}\" is not supported.", destExtension );
				return false;
			}
