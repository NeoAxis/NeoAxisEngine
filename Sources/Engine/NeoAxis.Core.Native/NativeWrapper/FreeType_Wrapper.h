...
struct Rect
{
	//Rect() : xmin(INT_MAX), xmax(INT_MIN), ymin(INT_MAX), ymax(INT_MIN) {}
	...
	int Width() const { return MAX(0, xmax - xmin + 1); }
	int Height() const { return MAX(0, ymax - ymin + 1); }
...
void RasterCallback(const int y, const int count, const FT_Span* const spans, void* const user) 
{
	...
	for (int i = 0; i < count; ++i)
	{
		...
		if (span.x > 65000) //if (span.x == 65536)
			continue;

		sptr->push_back(Span(span.x, y, span.len, span.coverage));
	}
}
...
EXPORT bool FreeType_GetGlyphData( FT_Library library, FT_Face face, int character, int bufferSizeX, int bufferSizeY, uint8* buffer, int* drawOffsetX, int* drawOffsetY, int* outSizeX, int* outSizeY, int* advance)
{
	*outSizeX = 0;
	*outSizeY = 0;
	*drawOffsetX = 0;
	...
	*advance = face->glyph->metrics.horiAdvance >> 6;
	...
	if (!spans.empty())
	{
		...
		{
			int x1 = s->x;
			int x2 = s->x + s->width - 1;

			rect.xmin = MIN(rect.xmin, x1);
			rect.xmax = MAX(rect.xmax, x2);
			rect.ymin = MIN(rect.ymin, s->y);
			rect.ymax = MAX(rect.ymax, s->y);
		}
		...
		if(buffer)
		{
			...
			for(Spans::iterator s = spans.begin(); s != spans.end(); ++s)
			{
				for(int w = 0; w < s->width; ++w)
				{
					...
					if( x >= 0 && y >= 0 && x < bufferSizeX && y < bufferSizeY )
					{
						uint8* pointer = buffer + (y * bufferSizeX + x);
...
EXPORT bool FreeType_GetGlyphContours(FT_Library library, FT_Face face, int character, FreeType_GetGlyphContoursDelegate callback, int* drawOffsetX, int* drawOffsetY, int* advance)
{
	*drawOffsetX = 0;
	*drawOffsetY = 0;
	*advance = 0;
