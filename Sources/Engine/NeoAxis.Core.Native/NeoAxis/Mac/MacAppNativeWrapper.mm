...
enum WindowStates
{
	WindowStates_Maximized,
	WindowStates_Minimized,
	WindowStates_Normal
};
...
EXPORT void* MacAppNativeWrapper_InitApplicationWindow(bool fullscreen, int windowSizeX, int windowSizeY, 
	const uint16* title)
{
	//NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
...
EXPORT int MacAppNativeWrapper_GetWindowState()
{
	if([mainWindow isMiniaturized] == YES)
		return WindowStates_Minimized;
	
	if([mainWindow isZoomed] == YES)
		return WindowStates_Maximized;
		
	return WindowStates_Normal;
}
...
EXPORT void MacAppNativeWrapper_SetWindowState(int state)
{
	...
	case WindowStates_Minimized:
		if([mainWindow isMiniaturized] == NO)
			[mainWindow miniaturize:nil];
		break;

	case WindowStates_Normal:
		if([mainWindow isMiniaturized] == YES)
		   [mainWindow deminiaturize:nil];
		if([mainWindow isZoomed] == YES)
			[mainWindow zoom:nil];
...
EXPORT void MacAppNativeWrapper_GetDisplayInfo(uint display, uint16** deviceName, Recti* bounds, 
	Recti* workingArea, bool* primary)
{
