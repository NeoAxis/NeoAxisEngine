...
#include "charset.h"
	...
	static const uint32_t kNumCharsPerBatch = 1024;
	static const uint32_t kNumBatchVertices = kNumCharsPerBatch*4;
	static const uint32_t kNumBatchIndices  = kNumCharsPerBatch*6;
	...
	void EncoderImpl::submit(ViewId _id, ProgramHandle _program, OcclusionQueryHandle _occlusionQuery, uint32_t _depth, uint8_t _flags)
	{
		...
		m_draw.m_bindIdx = renderItemIdx;
...
#undef BGFX_RENDERER_CONTEXT
		...
		, allocator(NULL)
	{
		...
		//__android_log_print(ANDROID_LOG_INFO, "bgfx", "XXXX bgfx_instance_data_buffer_t. %d", (int)sizeof(InstanceDataBuffer));
		//__android_log_print(ANDROID_LOG_INFO, "bgfx", "XXXX bgfx_attachment_t. %d", (int)sizeof(Attachment));
		//__android_log_print(ANDROID_LOG_INFO, "bgfx", "XXXX bgfx_caps_limits_s. %d", (int)sizeof(Caps));
		//__android_log_print(ANDROID_LOG_INFO, "bgfx", "XXXX bgfx_view_stats_s. %d", (int)sizeof(bgfx_view_stats_s));
		//__android_log_print(ANDROID_LOG_INFO, "bgfx", "XXXX bgfx_stats_t. %d", (int)sizeof(Stats));
	...
	bool init(const Init& _userInit)
	{
		...
		//!!!!betauser
		g_allocator = new bx::DefaultAllocator();
...
{
	return bx::vsnprintf(_str, (int32_t)_count, _format, _argList);
}

//!!!!betauser
bool global_tripleBuffering = false;
BGFX_C_API void bgfx_set_triple_buffering()
{
	global_tripleBuffering = true;
}

//!!!!betauser
char gpuDescription[256] = { 0 };
#define _MIN(x, y) (((x) < (y)) ? (x) : (y))
BGFX_C_API void bgfx_get_gpu_description(int bufferSize, char* result)
{
	memcpy(result, gpuDescription, _MIN(bufferSize, 256));
}

//!!!!betauser
BGFX_C_API void* bgfx_call_special(const char* name, void* parameter1, void* parameter2, void* parameter3, void* parameter4)
{
	BX_UNUSED(name);
...
//!!!!betauser
BGFX_C_API void bgfx_check_wrapper(int initSettings, int platformData, int nativeAttachment, int resolutionSize, int initLimitsSize, int statsSize, int capsSize)
{
	...
	if (sizeof(bgfx_resolution_s) != resolutionSize)
		Fatal2("bgfx_check_wrapper: sizeof(bgfx_resolution_s) != resolutionSize.");
	if (sizeof(bgfx_init_limits_s) != initLimitsSize)
		Fatal2("bgfx_check_wrapper: sizeof(bgfx_init_limits_s) != initLimitsSize.");
	if (sizeof(bgfx_stats_t) != statsSize)
		Fatal2("bgfx_check_wrapper: sizeof(bgfx_stats_t) != statsSize.");
	if (sizeof(bgfx_caps_s) != capsSize)
		Fatal2("bgfx_check_wrapper: sizeof(bgfx_caps_s) != capsSize.");
}
