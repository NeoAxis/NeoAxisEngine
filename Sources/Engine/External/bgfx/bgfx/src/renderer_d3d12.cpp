...
#if BX_PLATFORM_WINRT
				//!!!! we have LNK2019: unresolved external symbol D3D12GetDebugInterface referenced in function "public: bool __cdecl bgfx::d3d12::RendererContextD3D12::init(struct bgfx::Init const &)" (?init@RendererContextD3D12@d3d12@bgfx@@QEAA_NAEBUInit@3@@Z)
				//!!!! D3D12GetDebugInterface not supported in Win10 UWP SDK ?
				hr = -1;
				debug0 = NULL;
#else
		...
		void setBlendState(D3D12_BLEND_DESC& _desc, uint64_t _state, uint32_t _rgba = 0)
		{
			...
			if (_desc.IndependentBlendEnable)
			{
				...
				for (uint32_t ii = 0; ii < BGFX_CONFIG_MAX_FRAME_BUFFER_ATTACHMENTS; ++ii)
				{
					...
					writeMask2 |= (v & 1) ? D3D12_COLOR_WRITE_ENABLE_RED : 0;
					writeMask2 |= (v & 2) ? D3D12_COLOR_WRITE_ENABLE_GREEN : 0;
					writeMask2 |= (v & 4) ? D3D12_COLOR_WRITE_ENABLE_BLUE : 0;
					writeMask2 |= (v & 8) ? D3D12_COLOR_WRITE_ENABLE_ALPHA : 0;

					_desc.RenderTarget[ii].RenderTargetWriteMask = writeMask2;
				}
