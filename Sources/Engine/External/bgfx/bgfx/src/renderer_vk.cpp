...
#	endif
	...
	static void* VKAPI_PTR reallocationFunction(void* _userData, void* _original, size_t _size, size_t _alignment, VkSystemAllocationScope _allocationScope)
	{
		bx::AllocatorI* allocator = (bx::AllocatorI*)_userData;
		if (_size == 0) {
			bx::alignedFree(allocator, _original, 0);
			return NULL;
		}
		return bx::alignedRealloc(allocator, _original, _size, bx::max(kMinAlignment, _alignment), bx::Location(s_allocScopeName[_allocationScope], 0) );
	}
	...
	static void VKAPI_PTR freeFunction(void* _userData, void* _memory)
	{
		...
		bx::AllocatorI* allocator = (bx::AllocatorI*)_userData;
		bx::alignedFree(allocator, _memory, 0);
	}
...
#undef VK_IMPORT_DEVICE_FUNC
...

			{
				...
				if (!headless)
				{
					...
					result = m_backBuffer.create(UINT16_MAX, g_platformData.nwh, m_resolution.width, m_resolution.height, m_resolution.formatColor);
...
#	error "Figure out KHR surface..."
	...
	void RendererContextVK::submitBlit(BlitState& _bs, uint16_t _view)
	{
		...
		//!!!!betauser
		while (bs1.hasItem(_view) )
		{
			//!!!!betauser
			const BlitItem& blit = bs1.advance();
	...
	void RendererContextVK::submit(Frame* _render, ClearQuad& /*_clearQuad*/, TextVideoMemBlitter& _textVideoMemBlitter)
	{
		...
		if (0 == (_render->m_debug&BGFX_DEBUG_IFH) )
		{
			...
			for (int32_t item = 0; item < numItems;)
			{
				...
				const uint32_t bindIdx       = itemIdx;//isCompute ? renderItem.compute.m_bindIdx : renderItem.draw.m_bindIdx;
				const RenderBind& renderBind = _render->m_renderItemBind[bindIdx];
				++item;
