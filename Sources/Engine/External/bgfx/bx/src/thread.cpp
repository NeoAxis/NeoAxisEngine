...
// 1. Move the SEH logic into an isolated helper function with no C++ objects
#pragma pack(push, 8)
	struct ThreadName
	{
		DWORD  type;
...
#pragma pack(pop)
	...
	static void raiseThreadNameException(DWORD threadId, LPCSTR name)
	{
		ThreadName tn;
		tn.type = 0x1000;
		tn.name = name;
...
//!!!!betauser
		...
		raiseThreadNameException(ti->m_threadId, m_name.getCPtr());
