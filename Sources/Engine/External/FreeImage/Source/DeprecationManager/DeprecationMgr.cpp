...
#ifdef _WIN32
static int DereferencePointerToIntSafely(int* iptr)
{
#if defined(WINAPI_FAMILY) && (WINAPI_FAMILY & 2) != 0
	__try
	{
		return *iptr;
	}
...
#else
	return IsBadReadPtr(iptr, 4) ? 0 : *iptr;
#endif
}
#endif
// ==========================================================
...
DeprecationMgr::AddDeprecatedFunction(const char *old_function_name, const char *new_function_name, const void *frame_ptr) {
	...
	int called_from = DereferencePointerToIntSafely(preturn);
