...
namespace NeoAxis.Widget
{
	...
	public partial class EngineViewportControl : EUserControl
	{
		...
		protected override void OnPaint( PaintEventArgs e )
		{
			...
			if( Viewport != null && DrawSplashScreen )
			{
				using( var brush = new SolidBrush( Color.Black ) )
				{
					...
					var image = EngineInfo.SplashLogoImage;
		...
		static bool IsPhysicalVisibleCheckBy5Points( Control control )
		{
		...
		[Browsable( false )]
		internal bool DrawSplashScreen
		{
			get
			{
				if( IsWidget )
				{
					if( EngineApp.EngineTime != 0 )
					{
						...
						return EngineApp.EngineTime - splashScreenStartTime < totalTime;
					}
					else
						return true;
				}
