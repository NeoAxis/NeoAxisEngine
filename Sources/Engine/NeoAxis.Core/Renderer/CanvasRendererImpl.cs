...
namespace NeoAxis
{
	class CanvasRendererImpl : CanvasRenderer
	{
		...
		//!!!!!!
		//public void _AddToRenderIn3D( Viewport viewport )
		//{
		//	if( parentGuiSceneObjectFor3DRendering != null )
		//	{
		//		unsafe
		//		{
		//			//!!!!!!
		//			//RendererWorld.statistics.AddGuiRenderer();

		//			CheckForAllPushedParameters();

		//			int count = 0;
		//			foreach( Item item in outItems )
		//				count += item.renderableItems.Length;

		//			GuiRendererItemRenderable** renderables = (GuiRendererItemRenderable**)
		//				NativeUtils.Alloc( NativeUtils.MemoryAllocationType.Renderer, sizeof( GuiRendererItemRenderable* ) * count );
		...
		//				{
		//					renderables[ index ] = renderableItem.renderable;
		//					index++;

		//					//!!!!!
		//					//!!!!slowly?
		//					//RendererWorld.statistics.AddGuiBatch();
		//				}
		//			}

		//			//!!!!!
		//			//OgreGuiMovableObject.setPreparedRenderables( parentGuiSceneObjectFor3DRendering.realObject, count, (void**)renderables );

		//			NativeUtils.Free( (IntPtr)renderables );

		//			//if( !IsScreen )//for screen we need two times rendering (before scene and after)
		//			//{
		//			//   //Clear();
