...
namespace NeoAxis
{
	class Simple3DRendererImpl : Simple3DRenderer
	{
		...
		{
			//!!!!группой рисовать. получаетс€, могут быть вложенные группы
			...
			{
				//!!!!при толстых лини€х вроде как правильнее рисовать иначе, чтобы углы корректные были. где еще так?
		...
		unsafe void PrepareRenderables()
		{
			foreach( Item item in items )
			{
				...
				if( itemDynamicallyCreated != null )
				{
					...
					//!!!!можно дл€ всего юзать один буфер
