	...
	public class DocumentInstance
	{
		...
		internal protected virtual void OnShowTitleContextMenu( IDocumentWindow caller, KryptonContextMenuItems items )
		{
			...
			if( !string.IsNullOrEmpty( RealFileName ) )
			{
				...
				//if( isFirstLevel )
				{
					var item = new KryptonContextMenuItem( EditorLocalization.Translate( "General", "Reload" ), ( s, e ) =>
					{
						var document = caller.Document;
						...
						EditorAPI.CloseDocument( document, false );
						if( document.Destroyed )
							EditorAPI.OpenFileAsDocument( document.RealFileName, true, true, specialMode: document.SpecialMode );
