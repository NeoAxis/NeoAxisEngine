	...
	public partial class StoresWindow : DockWindow
	{
		...
		public StoresWindow()
		{
			...
			foreach( var item in toolStripForTreeView.Items )
			{
				...
				if( button != null )
					button.Text = EditorLocalization.Translate( "StoresWindow", button.Text );

				var button2 = item as ToolStripDropDownButton;
				if( button2 != null )
					button2.Text = EditorLocalization.Translate( "StoresWindow", button2.Text );
			}
		...
		{
			return EditorLocalization.Translate( "StoresWindow", text );
		}
...

						{
							...
							var item = new KryptonContextMenuItem( Translate( "Go to Folder" ), EditorResourcesCache.SelectFolder, delegate ( object s, EventArgs e2 )
							{
								if( !string.IsNullOrEmpty( topDirectory ) )
									EditorAPI.SelectFilesOrDirectoriesInMainResourcesWindow( new string[] { topDirectory }, true );
							} );
