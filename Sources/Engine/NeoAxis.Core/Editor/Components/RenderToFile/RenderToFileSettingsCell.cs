...
namespace NeoAxis.Editor
{
	public class RenderToFileSettingsCell : SettingsCellProcedureUI
	{
		...
		void RenderByEngineRender()
		{
			...
			if( string.IsNullOrEmpty( destRealFileName ) )
			{
				...
				case RenderToFile.ModeEnum.Screenshot:
					if( !EditorUtility.ShowSaveFileDialog( "", "Output.png", "PNG files (*.png)|*.png", out destRealFileName ) )
						return;
					break;

				case RenderToFile.ModeEnum.Video:
					if( !EditorUtility.ShowSaveFileDialog( "", "Output.avi", "AVI files (*.avi)|*.avi", out destRealFileName ) )
						return;
					break;

				case RenderToFile.ModeEnum.Material:
					if( !EditorUtility.ShowSaveFileDialog( "", "Output.material", "Material files (*.material)|*.material", out destRealFileName ) )
						return;
					break;
				}
			}
		...
		void RenderVideo( Camera camera, RenderingPipeline renderingPipeline, string destRealFileName )
		{
			...
			var document = EditorAPI.GetDocumentByObject( component );
			if( document == null )
				return;
