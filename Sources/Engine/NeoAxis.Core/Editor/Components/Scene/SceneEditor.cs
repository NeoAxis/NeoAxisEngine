...
namespace NeoAxis.Editor
{
	public partial class SceneEditor : DocumentWindowWithViewport
	{
		...
		{
			(var objectType, var referenceToObject, var anyData, var objectName) = EditorAPI.GetObjectToCreateByDropData( e );
		...
		void CreateSetProperty_Update( DragEventArgs dragEventArgs )
		{
			...
			ContentBrowser.Item contentBrowserItem = null;
			{
				...
				if( CreateObjectsMode == CreateObjectsModeEnum.Click )
					contentBrowserItem = EditorAPI.CreateObjectGetSelectedContentBrowserItem();
			}
		...
		protected override void GetTextInfoLeftTopCorner( List<string> lines )
		{
			...
			if( Scene != null && Scene.GetDisplayDevelopmentDataInThisApplication() && Scene.DisplayTextInfo )
			{
				...
				if( statistics != null )
				{
					...
					lines.Add( Translate( "Draw calls" ) + ": " + statistics.DrawCalls.ToString( "N0" ) );
		...
		void UpdateCreateObjectsDestinationCachedList()
		{
			if( Time.Current > createObjectsDestinationLastUpdateTime + 1.0 )
			{
				...
				if( CreateObjectsMode == CreateObjectsModeEnum.Drop || CreateObjectsMode == CreateObjectsModeEnum.Click )
				{
					...
					createObjectsDestinationCachedList.Add( (CreateObjectsDestinationModeEnum.SeparateObjectsToRoot, null, EditorLocalization.Translate( "General", "to Root" )) );
				...
				else
				{
					...
					createObjectsDestinationCachedList.Add( (CreateObjectsDestinationModeEnum.SeparateObjectsToRoot, null, EditorLocalization.Translate( "General", "Separate objects to Root" )) );
					foreach( var obj in Scene.GetComponents<Layer>( checkChildren: true, onlyEnabledInHierarchy: true ) )
					{
						if( obj.DisplayInEditor )
		...
		{
			(Metadata.TypeInfo objectType, string referenceToObject, object anyData, string objectName) result = EditorAPI.GetSelectedObjectToCreate();
