...
namespace NeoAxis.Editor
{
						...
						if( !itemsToHide.Contains( name.ToLower() ) )
						{
							...
							if( item == null )
							{
								...
								try
								{
									...
									if( extension == ".cs" || extension == ".csscript" )
										item.imageKey = "CSharp";
									else if( extension == ".settings" )
										item.imageKey = "Cog";
									else
									{
										var resourceType = ResourceManager.GetTypeByFileExtension( extension );
										if( resourceType != null )
										{
											if( resourceType.Name == "Image" )
												item.imageKey = "Image";
											else if( resourceType.Name == "UI" )
												item.imageKey = "UI";
											else if( resourceType.Name == "Sound" )
												item.imageKey = "Sound";
											else if( resourceType.Name == "Mesh" || resourceType.Name == "Import 3D" )
												item.imageKey = "Mesh";
					...
					if( ResourceManager.GetTypeByFileExtension( ext ) != null )//!!!!надо?
					{
						...
						if( res != null && res.PrimaryInstance != null )
						{
							...
							if( obj != null )
							{
								...
								{
									//specialization for Component_Import3D
									if( obj is Component_Import3D && expectedType == MetadataManager.GetTypeOfNetType( typeof( Component_Mesh ) ) )
									{
										var mesh = obj.GetComponent( "Mesh" ) as Component_Mesh;
										if( mesh != null )
										{
											canSet = true;
											referenceValue += "|$Mesh";
	...
	public class ContentBrowserItem_Component : ContentBrowser.Item
	{
			...
			: base( owner, parent )
		{
			...
			component.NameChanged += Component_NameChanged;
			component.EnabledInHierarchyChanged += Component_EnabledInHierarchyChanged;
			component.ComponentsChanged += Component_ComponentsChanged;
			...
			get
			{
				...
				if( string.IsNullOrEmpty( SpecialTextPrefix ) )
				{
					if( nameText == "" && component.Parent == null )
						nameText = "\'" + EditorLocalization.Translate( "General", "Root" ) + "\'";
					if( nameText == "" )
						nameText = "\'" + EditorLocalization.Translate( "General", "No name" ) + "\'";
				}
		...
		private void Component_NameChanged( Component obj )
		{
		...
		private void Component_EnabledInHierarchyChanged( Component obj )
		{
			if( EditorAPI.ClosingApplication )
				return;
		...
		private void Component_ComponentsChanged( Component obj )
		{
			if( EditorAPI.ClosingApplication )
				return;
