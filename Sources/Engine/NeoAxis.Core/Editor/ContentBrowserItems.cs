...
namespace NeoAxis.Editor
{
	public class ContentBrowserItem_File : ContentBrowser.Item
	{
		...
		public override Image[] GetCornerImages()
		{
			...
			{
				var projectFileName = VirtualPathUtility.GetProjectPathByReal( fullPath );

				if( IsDirectory )
				{
					//!!!!only Assets folder
					var assetsFolder = VirtualFileSystem.Directories.Assets;
					if( fullPath.Length >= assetsFolder.Length && string.Compare( fullPath.Substring( 0, assetsFolder.Length ), assetsFolder, true ) == 0 )
					{
