...
namespace NeoAxis.Editor
{
	internal class HCDropDownHolderImpl
	{
		...
		{
			//if( m.Msg == PI.WM_NCACTIVATE && m.WParam != IntPtr.Zero && _childPopup != null && _childPopup.Visible )
			//{
			//	_childPopup.Hide();
			//}
		...
		private bool OnNcHitTest( ref Message m, bool contentControl )
		{
			...
			IntPtr transparent = new IntPtr( PI.HTTRANSPARENT );

			if( resizableTop )
			{
				if( resizableLeft && gripBouns.TopLeft.Contains( clientLocation ) )
				{
					m.Result = contentControl ? transparent : (IntPtr)PI.HTTOPLEFT;
					return true;
				}
				if( !resizableLeft && gripBouns.TopRight.Contains( clientLocation ) )
				{
					m.Result = contentControl ? transparent : (IntPtr)PI.HTTOPRIGHT;
					return true;
				}
				if( gripBouns.Top.Contains( clientLocation ) )
				{
					m.Result = contentControl ? transparent : (IntPtr)PI.HTTOP;
					return true;
				}
			}
			else
			{
				...
				{
					m.Result = contentControl ? transparent : (IntPtr)PI.HTBOTTOMLEFT;
					return true;
				}
				if( !resizableLeft && gripBouns.BottomRight.Contains( clientLocation ) )
				{
					m.Result = contentControl ? transparent : (IntPtr)PI.HTBOTTOMRIGHT;
					return true;
				}
				if( gripBouns.Bottom.Contains( clientLocation ) )
				{
					m.Result = contentControl ? transparent : (IntPtr)PI.HTBOTTOM;
					return true;
				}
			}
			if( resizableLeft && gripBouns.Left.Contains( clientLocation ) )
			{
				m.Result = contentControl ? transparent : (IntPtr)PI.HTLEFT;
				return true;
			}
			if( !resizableLeft && gripBouns.Right.Contains( clientLocation ) )
			{
				m.Result = contentControl ? transparent : (IntPtr)PI.HTRIGHT;
				return true;
			}
			return false;
		}
