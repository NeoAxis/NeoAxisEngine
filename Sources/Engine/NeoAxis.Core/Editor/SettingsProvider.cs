	...
	public class SettingsProvider
	{
		...
		protected virtual void OnUpdate()
		{
			for( int n = 0; n < selectedObjects.Length; n++ )
			{
				...
				foreach( var attr in obj.GetType().GetCustomAttributes<EditorSettingsCellAttribute>( true ).Reverse() )
				{
				...
				//		{
				//			foreach( var cellComponent in componentType.BasedOnObject.GetComponents<Component_SettingsCell>( onlyEnabledInHierarchy: true ) )
				//			{
				//				if( selectedObjects.Length == 1 /*|| attr.MultiselectionSupport*/ )
				//				{
				//					var cell = new Component_SettingsCell.SettingsCellImpl( cellComponent );
				//					AddCell( cell );
				//				}
				//			}
				//		}
				...
				//	foreach( var cellComponent in component.GetComponents<Component_SettingsCell>( onlyEnabledInHierarchy: true ) )
				//	{
				//		if( selectedObjects.Length == 1 /*|| attr.MultiselectionSupport*/ )
				//		{
				//			var cell = new Component_SettingsCell.SettingsCellImpl( cellComponent );
