				...
				if( contentBrowserItem != null )
				{
					...
					if( cornerImages != null )
					{
						for( int n = 0; n < cornerImages.Length; n++ )
						{
							...
							if( cornerImage != null )
							{
								e.Graphics.DrawImage( cornerImage, ( rect.Left + rect.Right ) / 2 - imageSize / 2 + imageSize * n / 3, rect.Top + (int)( EditorAPI.DPIScale * 5.0f ) + imageSize * 2 / 3, imageSize / 3, imageSize / 3 );
