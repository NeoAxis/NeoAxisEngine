...
namespace Internal//NeoAxis
{
	...
	/// </summary>
	public class DebugInfo_RenderResources : DebugInfo
	{
		...
		public override List<string> Content
		{
			get
			{
				...
				foreach( var texture in GpuTexture.GetAll() )
				{
						...
						if( data != null )
						{
							foreach( var surfaceData in data )
							{
								if( surfaceData.Data != null )
									texturesSizeManaged += surfaceData.Data.Count;// Length;
							}
						}
				...
				foreach( var mesh in Mesh.GetAll() )
				{
					meshes++;
