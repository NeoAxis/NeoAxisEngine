...
//	{
//		internal const int maxMessageTypeIdentifier =
//			( 1 << NetworkDefines.bitsForServiceMessageTypeIdentifier ) - 1;
...
//		List<MessageType> messageTypesByIdentifier = new List<MessageType>();
...
//			public delegate bool ReceiveHandlerDelegate( NetworkNode.ConnectedNode sender,
//				MessageType messageType, ArrayDataReader reader, ref string additionalErrorMessage );
...
//			MessageType.ReceiveHandlerDelegate receiveHandler )
//		{
//			if( identifier == 0 )
//			{
//				Log.Fatal( "NetworkService: RegisterMessageType: Invalid message type identifier. " +
...
//			{
//				Log.Fatal( "NetworkService: RegisterMessageType: Invalid message type identifier. " +
//					"Max identifier is \"{0}\".", maxMessageTypeIdentifier );
//			}

//			if( GetMessageType( name ) != null )
//			{
//				Log.Fatal( "NetworkService: RegisterMessageType: Message type \"{0}\" " +
//					"is already registered.", name );
//			}

//			if( GetMessageType( identifier ) != null )
//			{
//				Log.Fatal( "NetworkService: RegisterMessageType: Message type with identifier \"{0}\" " +
//					"is already registered.", identifier );
//			}

//			MessageType messageType = new MessageType( name, identifier, receiveHandler );
//			messageTypesByName.Add( name, messageType );

//			while( messageTypesByIdentifier.Count <= identifier )
//				messageTypesByIdentifier.Add( null );
//			messageTypesByIdentifier[ identifier ] = messageType;
...
//		{
//			if( identifier >= messageTypesByIdentifier.Count )
//				return null;
//			return messageTypesByIdentifier[ identifier ];
//		}

//		internal void ProcessReceivedMessage( NetworkNode.ConnectedNode connectedNode,
...
//			if( !messageType.ReceiveHandler( connectedNode, messageType, reader, 
//				ref additionalErrorMessage ) )
//			{
//				string text = string.Format( "Invalid service message \"{0}\"", messageType.Name );
//				if( !string.IsNullOrEmpty( additionalErrorMessage ) )
//					text += string.Format( " ({0})", additionalErrorMessage );
