...
//			var m = BeginMessage( sender, GetMessageType( "SendFilesToClient" ) );
//			var writer = m.Writer;
//			var totalSent = 0;
...
//						writer.Write( fileItem.TimeModified );//!!!!каждый блок необ€зательно
...
//		//{
//		//	//!!!!кешировать? если много раз вызываетс€, то надо
...
//				//если на сервере и в локальном сторедже нет, а он есть, то удал€ть
...
//				//если на сервере есть, а тут нет, или различаютс€, то скачивать
//				foreach( var info in filesOnServer.Values )
//				{
//					var fullPath = Path.Combine( DestinationFolder, info.FileName );
...
//					info.Hash = reader.ReadString() ?? string.Empty;
...
//				//else
//				//{
//				//	//!!!!провер€ть еще раз? хеш всех хешей может
...
//			//!!!!провер€ть еще раз? хеш всех хешей может
//			//!!!!лочить файлы, чтобы нельз€ было мен€ть вручную в процессе работы?
...
//						//может влазить несколько блоков одного файла. сейчас получаетс€ несколько файлов одновременно будет качать
...
//					var m = BeginMessage( GetMessageType( "RequestFilesToServer" ) );
//					var writer = m.Writer;
...
//			var m = BeginMessage( GetMessageType( "RequestInfoToServer" ) );
//			m.End();
//		}
//	}
//}
