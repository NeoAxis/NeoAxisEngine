...
namespace NeoAxis
{
	...
	public class ServerNetworkService_Components : ServerService
	{
		...
		public void SetScene( Scene scene, string sceneInfo, double simulationStepsPerSecond )
		{
			ResetScene( false );
			...
			scene.HierarchyController.networkServerNode = Owner;

			//!!!!возможно не всем высылать
			//send initial scene state to all clients
			SendSceneCreate( null, simulationStepsPerSecond );
		}

		public void ResetScene( bool anotherSceneWillLoaded )
		{
			if( scene != null )
			{
				...
				SendSceneDestroy( null, anotherSceneWillLoaded );
			}
		}
		...
		/// <summary>
		/// Get all clients with ServerNode.Client. Bots are not included.
		/// </summary>
		/// <returns></returns>
		[MethodImpl( MethodImplOptions.AggressiveInlining | (MethodImplOptions)512 )]
		public ClientItem[] GetAllClientItems()
		{
			...
			{
				allClientItemsCached = clientItemByUser.Values.Where( v => v.User.Client != null ).ToArray();
		...
		BeginMessageContext BeginMessage( ICollection<ClientItem> clients, byte messageID )
		{
			...
			if( clients is List<ClientItem> list )
			{
				for( int n = 0; n < list.Count; n++ )
					m.Recipients.Add( list[ n ].User.Client );
			}
			else if( clients is ClientItem[] array )
			{
				foreach( var client in array )
					m.Recipients.Add( client.User.Client );
			}
			else
			{
		...
		public void SendSceneCreate( ClientItem clientItem, double simulationStepsPerSecond )
		{
			if( scene == null )
			...
			try
			{
				...
				//scene create end
				{
					...
					m.Writer.Write( Owner.Interpolation );
					m.Writer.Write( (float)simulationStepsPerSecond );
					m.End();
		...
		public void SendSceneDestroy( ClientItem clientItem, bool anotherSceneWillLoaded )// ServerNetworkService_Users.UserInfo user )
		{
			var m = BeginMessage2( clientItem, SceneDestroyToClient );
			if( m != null )
			{
				m.Writer.Write( anotherSceneWillLoaded );
				m.End();
			}
		...
		private void NetworkInterface_BeginNetworkMessage( ComponentHierarchyController.NetworkServerInterface sender, Component component, IList<ClientItem> clientRecipients, ClientItem clientRecipient, IList<ServerNetworkService_Users.UserInfo> userRecipients, ServerNetworkService_Users.UserInfo userRecipient, bool toEveryone, string message, ref BeginMessageContext messageContext )
		{
			if( component.networkID != 0 && !component.Disposed && !component.RemoveFromParentQueued )
			{
				...
				for( int n = 0; n < _tempNetworkMessageRecepients.Count; n++ )
					messageContext.Recipients.Add( _tempNetworkMessageRecepients[ n ].User.Client );
	...
	public class ClientNetworkService_Components : ClientService
	{
		...
		public delegate void SceneDestroyDelegate( ClientNetworkService_Components sender, bool anotherSceneWillLoaded );
		...
		const int ComponentSendNetworkMessageName = 10;//both directions
		const int ComponentSendNetworkMessage = 11;//both directions
		internal const int SimulationStepToClient = 12;
		...
		bool ReceiveMessage_SceneCreateBeginToClient( MessageType messageType, ArrayDataReader reader, ref string additionalErrorMessage )
		{
			...
			Owner.SetInterpolation( false, 0, null );
			...
			scene.HierarchyController.networkClientNode = Owner;
		...
		bool ReceiveMessage_SceneCreateEndToClient( MessageType messageType, ArrayDataReader reader, ref string additionalErrorMessage )
		{
			...
			var interpolation = reader.ReadBoolean();
			var interpolationStepsPerSecond = reader.ReadSingle();
			if( !reader.Complete() )
				return false;
			...
			if( scene != null )
				Owner.SetInterpolation( interpolation, interpolationStepsPerSecond, scene );
			else
				Owner.SetInterpolation( false, 0, null );
		...
		{
			var anotherSceneWillLoaded = reader.ReadBoolean();
			if( !reader.Complete() )
				return false;
			...
			SceneDestroy?.Invoke( this, anotherSceneWillLoaded );
		...
		bool ReceiveMessage_SimulationStepToClient( MessageType messageType, ArrayDataReader reader, ref string additionalErrorMessage )
		{
			...
			if( scene != null )
			{
				...
				//Log.Info( ( EngineApp.EngineTime - prev ).ToString( "0.000" ) );
				//prev = EngineApp.EngineTime;
			}
