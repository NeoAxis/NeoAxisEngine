...
namespace NeoAxis.Networking
{
	public abstract class NetworkServerNode : NetworkNode
	{
		...
		public delegate void IncomingConnectionApprovalDelegate( NetworkServerNode sender, ConnectedNode connectedNode, string clientVersion, string loginData,/* string loginName, string password,*/ /*ref bool approve, */ref string rejectReason );
				...
				case NetIncomingMessageType.ConnectionApproval:
					{
						...
						try
						{
							...
							if( count > 100 )
							{
								incomingMessage.SenderConnection.Deny( "More than 100 remote services." );
								return;
							}
						...
						var isNewNode = connectedNode == null;

						//new connection
						if( connectedNode == null )
						{
							////skip equal IPEndPoint. this is Lidgren bug?   it just support double connection
							//{
							//	bool skip = false;
							//	foreach( var node in GetConnectedNodesArray() )
							//	{
						...
						var rejectReason = "";
						if( OnIncomingConnectionApproval( connectedNode, clientVersion, loginData, ref rejectReason ) )
						{
							//approve
		...
		protected virtual bool OnIncomingConnectionApproval( ConnectedNode connectedNode, string clientVersion, string loginData, /*string loginName, string password, */ref string rejectReason )
		{
			...
			if( !Disposed )
			{
				IncomingConnectionApproval?.Invoke( this, connectedNode, clientVersion, loginData,/* loginName, password,*/ ref rejectReason );
		...
		{
			get { return server.ConnectionsCount; }
		}
