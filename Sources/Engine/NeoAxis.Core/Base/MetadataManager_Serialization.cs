		...
		public static class Serialization
		{
			...
			static void RegisterSimpleTypes()
			{
				foreach( SimpleTypes.TypeItem item in SimpleTypes.Types )
				{
					...
					{
						//!!!!string
						if( type.GetNetType() == typeof( string ) && string.IsNullOrEmpty( value as string ) )
							return;
					...
					if( b != null )
					{
						...
						if( count != 0 )
						{
							...
							if( value is int[] )
							{
								...
								if( !string.IsNullOrEmpty( str ) )
								{
									if( !StringUtility.StringToInts( str, (int[])value ) )
									{
					...
					delegate ( Metadata.SaveContext context, Metadata.TypeInfo type, string parameterName, TextBlock block, object value, bool defaultValueSpecified, object defaultValue, ref string error )
				{
					...
					if( value != null )
					{
						...
						if( count != 0 )
						{
							...
							if( value is byte[] )
							{
								string str = Convert.ToBase64String( (byte[])value, Base64FormattingOptions.None );
