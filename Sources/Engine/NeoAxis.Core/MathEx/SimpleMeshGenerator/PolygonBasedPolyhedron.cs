...
namespace NeoAxis
{
	...
	{
		public static void GeneratePolygonBasedPolyhedron( Vector3[] points, bool clockwise, double height, bool insideOut, Vector3? normal, out Vector3[] positions, out Vector3[] normals, out Vector4[] tangents, out Vector2[] texCoords, out int[] indices, out Face[] faces )
		{
			if( points.Length < 3 )
			{
			...
			var normal2 = Plane.FromPoints( points[ 0 ], points[ 1 ], points[ 2 ] ).Normal;
			if( normal.HasValue )
				normal2 = normal.Value;
			if( clockwise )
			{
				normal2 = -normal2;
				insideOut = !insideOut;
			...
			Matrix4[] m = GetTransformsOfLinePath( segmentsHeight + 1, Vector3.Zero, normal2 * height );
		...
		public static void GeneratePolygonBasedPolyhedron( Vector3[] points, bool clockwise, double height, bool insideOut, Vector3? normal, out Vector3F[] positions, out Vector3F[] normals, out Vector4F[] tangents, out Vector2F[] texCoords, out int[] indices, out Face[] faces )
		{
			GeneratePolygonBasedPolyhedron( points, clockwise, height, insideOut, normal, out Vector3[] positionsD, out Vector3[] normalsD, out Vector4[] tangentsD, out Vector2[] texCoordsD, out indices, out faces );
