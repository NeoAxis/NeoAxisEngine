	...
	public static class EngineConsole
	{
		...
		public static void PerformRenderUI()
		{
			...
			for( int n = startpos; n >= 0 && y - fontheight > 0; n-- )
			{
				...
				{
					foreach( var lineItem in font.GetWordWrapLines( fontSize, renderer, text2, 1.0 - x ).GetReverse() )
					{
		...
		static void ConsoleWindowedMode( string arguments )
		{
			if( !string.IsNullOrEmpty( arguments ) )
			{
				try
				{
					WindowedModeEnum value;
					if( arguments == "1" )
						value = WindowedModeEnum.Windowed;
					else if( arguments == "0" )
						value = WindowedModeEnum.Fullscreen;
					else
					{
						if( !Enum.TryParse( arguments, true, out value ) )
						{
							Print( string.Format( "Invalid value. Possible values: \"{0}\", \"{1}\", \"2\".", WindowedModeEnum.Fullscreen, WindowedModeEnum.Borderless, WindowedModeEnum.Windowed ) );
							return;
						}
					}

					EngineApp.SetWindowedMode( value, EngineApp.WindowedModeSize );
				}
				catch( Exception e )
				{
				...
				Print( string.Format( "Value: \"{0}\", Default value: \"{1}\"", EngineApp.WindowedMode, WindowedModeEnum.Fullscreen ) );
		}
		...
		static void ConsoleVideoMode( string arguments )
		{
			if( !string.IsNullOrEmpty( arguments ) )
			{
				try
				{
					var mode = Vector2I.Parse( arguments );

					if( EngineApp.WindowedMode == WindowedModeEnum.Fullscreen && !SystemSettings.VideoModeExists( mode ) )
					{
					...
					EngineApp.SetWindowedMode( EngineApp.WindowedMode, mode );
				}
				catch( Exception e )
				{
					Log.Warning( e.Message );
				...
				Print( string.Format( "Value: \"{0}\"", EngineApp.WindowedModeSize ) );
		}
