                                              ...
                                              Color remapNew)
		{
            ...
            lock (_threadLock)
            {
                ...
                try
                {
                    ...
                    // 3 variants:
                    // 1. dont scale, dont align
                    //      context.Graphics.DrawImage(image, imageRect, 0, 0, imageRect.Width, imageRect.Height, GraphicsUnit.Pixel, attribs)
                    // 2. scale
                    //      context.Graphics.DrawImage(image, imageRect, 0, 0, image.Width, image.Height, GraphicsUnit.Pixel, attribs);
                    // 3. align to center or downscale :

                    if (imageRect.Size == image.Size)
                    {
                        // dont scale, dont align
                        context.Graphics.DrawImage(image, imageRect, 0, 0, imageRect.Width, imageRect.Height, GraphicsUnit.Pixel, attribs);
                    }
                    else if(imageRect.Width < image.Width || imageRect.Height < image.Height)
                    {
                        // downscale and align
                        int size = Math.Min(imageRect.Width, imageRect.Height);
                        var rect = new Rectangle(imageRect.X - (imageRect.Width - size) / 2, imageRect.Y + (imageRect.Height - size) / 2, size, size);
                        var mode = context.Graphics.InterpolationMode; // should we restore the InterpolationMode ?
                        context.Graphics.InterpolationMode = InterpolationMode.HighQualityBicubic;
                        context.Graphics.DrawImage(image, rect, 0, 0, image.Width, image.Height, GraphicsUnit.Pixel, attribs);
                        context.Graphics.InterpolationMode = mode;
                    }
                    else
                    {
                        // align
                        var loc = imageRect.Location + new Size((imageRect.Width - image.Width) / 2, (imageRect.Height - image.Height) / 2);
                        context.Graphics.DrawImage(image, new Rectangle(loc, image.Size), 0, 0, image.Width, image.Height, GraphicsUnit.Pixel, attribs);
                    }
                }
