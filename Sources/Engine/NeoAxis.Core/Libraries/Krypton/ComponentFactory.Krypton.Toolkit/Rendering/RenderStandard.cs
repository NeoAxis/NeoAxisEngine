...
namespace ComponentFactory.Krypton.Toolkit
{
	...
	public class RenderStandard : RenderBase
    {
                                                  ...
                                                  bool glowing)
		{
            ...
            // HACK:
            // DPI scale depended text rect correction for Button
            if (context.Control is KryptonButton && DpiHelper.Default.DpiScaleFactor > 1.0f)
            {
                memento.ShortTextRect.Y += 1;
                memento.LongTextRect.Y += 1;
            }
            //
                                                   ...
                                                   IDisposable memento)
        {
            ...
            switch (palette.GetRibbonBackColorStyle(state))
            {
				...
				case PaletteRibbonColorStyle.RibbonGroupAreaBorder2016:
					return DrawRibbonGroupAreaBorder2016(context, rect, state, palette, memento);
                                          ...
                                          bool pressed)
        {
            ...
            {
				//DrawCheckBoxFromCode(context, displayRect, palette, enabled, checkState, tracking, pressed);
				//return;
				...
				//float scale = DpiHelper.Default.DpiScaleFactor;
				//context.Graphics.DrawImage(drawImage, 
				//	new Rectangle(
				//		displayRect.X + xOffset,
				//		displayRect.Y + yOffset,
				//		DpiHelper.Default.ScaleValue(drawImage.Width),
				//		DpiHelper.Default.ScaleValue(drawImage.Height)),
				//	0, 0, drawImage.Width, drawImage.Height, GraphicsUnit.Pixel);
			}
        }

		//private PaletteState GetCheckBoxPaletteState(bool enabled,
		//								  CheckState checkState,
		//								  bool tracking,
		//								  bool pressed)
		//{
		...
		//private void DrawCheckBoxFromCode(RenderContext context,
		//								  Rectangle displayRect,
		//								  IPalette palette,
		//								  bool enabled,
		//								  CheckState checkState,
		...
		//								  bool pressed)
		//{
		//	var state = GetCheckBoxPaletteState(enabled, checkState, tracking, pressed);
		//	Color _borderColor = palette.GetBorderColor1(PaletteBorderStyle.ButtonButtonSpec, state);
		//	Color _backColor = palette.GetBackColor1(PaletteBackStyle.ButtonButtonSpec, state);
		//	Color _imageColor = pressed ? Color.FromArgb(80, 80, 80) : Color.FromArgb(126, 126, 126);

		//	displayRect = new Rectangle(
		//		DpiHelper.Default.ScaleValue(displayRect.X),
		//		DpiHelper.Default.ScaleValue(displayRect.Y),
		//		DpiHelper.Default.ScaleValue(displayRect.Width),
		//		DpiHelper.Default.ScaleValue(displayRect.Height));
		...
		//		context.Graphics.DrawRectangle(pen, displayRect);

		//	// image 

		//	if (checkState == CheckState.Checked)
		//	{
		//		var imageColor = _imageColor;
		//		if (checkState == CheckState.Indeterminate)
		//		{
		//			using (SolidBrush b = new SolidBrush(imageColor))
		//			{
		//				Rectangle innerRect = displayRect;
		//				innerRect.Inflate(-3, -3);
		//				context.Graphics.FillRectangle(b, innerRect);
		//			}
		//		}
		//		else
		//		{
		//			// ver1
		//			//using (var pen = new Pen(imageColor, 2))
		//			//	e.Graphics.DrawLines(pen, CheckmarkLine);

		//			// ver2
		//			using (Bitmap bitmap = DrawCheckMarkBitmap(imageColor, displayRect))
		//				context.Graphics.DrawImage(bitmap, displayRect);
		//		}
		//	}
		//}
		...
		//	new Point(6, 10),
		//	new Point(12, 4) };
		//private static readonly Point[] CheckmarkLine = { new Point(2, 8), new Point(6, 12), new Point(13, 5) };

		//private Bitmap DrawCheckMarkBitmap(Color color, Rectangle displayRect)
		//{
		//	var checkMark = new Bitmap(15, 15 + 2);
            ...
            switch (shape)
            {
                ...
                case PaletteRibbonShape.Office2010:
                    using (SolidBrush fillBrush = new SolidBrush(darkColor))
                    {
                        ...
                        {
                            context.Graphics.FillPolygon(fillBrush, new Point[]{ new Point(displayRect.Left - 2, displayRect.Top - 1),
                                                                             new Point(displayRect.Left + 2, displayRect.Top + 4),
                                                                             new Point(displayRect.Left + 6, displayRect.Top) });
                        }
                        else
                        {
                                                    ...
                                                    bool qatAbove)
        {
            ...
            using (Pen darkPen = new Pen(c1), lightPen = new Pen(c2))
            {
                    if (shape == PaletteRibbonShape.Office2010)
                    {
                    context.Graphics.DrawLine(lightPen, displayRect.Left - 1, displayRect.Top, displayRect.Left + 5, displayRect.Top);
                    context.Graphics.DrawLine(darkPen, displayRect.Left - 1, displayRect.Top + 1, displayRect.Left + 5, displayRect.Top + 1);
                    }
                    else
                    {
                    context.Graphics.DrawLine(lightPen, displayRect.Left, displayRect.Top, displayRect.Left + 4, displayRect.Top);
                    context.Graphics.DrawLine(darkPen, displayRect.Left, displayRect.Top + 1, displayRect.Left + 4, displayRect.Top + 1);
                    }

                context.Graphics.DrawLine(lightPen, displayRect.Left, displayRect.Top + 3, displayRect.Left + 4, displayRect.Top + 3);
                context.Graphics.DrawLine(lightPen, displayRect.Left + 1, displayRect.Top + 4, displayRect.Left + 3, displayRect.Top + 4);
                context.Graphics.DrawLine(lightPen, displayRect.Left + 2, displayRect.Top + 4, displayRect.Left + 2, displayRect.Top + 5);
                context.Graphics.DrawLine(darkPen, displayRect.Left, displayRect.Top + 4, displayRect.Left + 2, displayRect.Top + 6);
                context.Graphics.DrawLine(darkPen, displayRect.Left + 2, displayRect.Top + 6, displayRect.Left + 4, displayRect.Top + 4);
            }
        }
                                                      ...
                                                      PaletteState state)
        {
            ...
            switch (shape)
            {
                default:
                case PaletteRibbonShape.Office2007:
                    using (Pen darkPen = new Pen(Color.Red),
                               lightPen = new Pen(lightColor))
                    {
                                                           ...
                                                           int allocatedHeight)
        {
            ...
            if (kryptonForm != null)
            {
                ...
                Padding chromeBorders = kryptonForm.WindowMargin;
        ...
        public static readonly object HighDpiReady = "HighDpiReady";
        ...
        public static Image GetImageForDispalyScale(Image image16px, Image image32px)
        {
            ...
            return GetImageForDispalyScale(image16px, image32px, 16);
        }

        public static Image GetImageForDispalyScale(Image imageSmall, Image imageBig, float scaleMultiplier)
        {
            if (imageSmall == null)
                return null;

            Image result;
            if (DpiHelper.Default.DpiScaleFactor >= 2.0)
                result = imageBig;
            else if (DpiHelper.Default.DpiScaleFactor >= 1.5)
                result = DpiHelper.Default.ScaleBitmapToSize((Bitmap)imageBig, (int)(scaleMultiplier * DpiHelper.Default.DpiScaleFactor));
            else
                result = imageSmall;

            result.Tag = RenderStandard.HighDpiReady;
            return result;
        }
                                               ...
                                               ref Size[,] allocation)
        {
            ...
            if (memento.Image != null)
            {
                try
                {
                    // Cache the size of the image and scale at high dpi.

                    // scale image rect based on image rect and center image (window buttons)
                    // scale image rect based on display rect and downscale image (backstage back)

                    if (DpiHelper.Default.IsScalingRequired && memento.Image.Tag != HighDpiReady)
                        memento.ImageRect.Size = DpiHelper.Default.ScaleValue(memento.Image.Size);
                    else
                        memento.ImageRect.Size = memento.Image.Size;

                    if ((memento.ImageRect.Width > displayRect.Width) || (memento.ImageRect.Height > displayRect.Height))
                        memento.ImageRect.Size = displayRect.Size;
                                                   ...
                                                   bool glowing)
        {
            ...
            if ((shortText != null) && (shortText.Length > 0))
            {
                ...
                memento.ShortTextMemento = AccurateText.MeasureString(g,
                                                                      ...
                                                                      paletteContent.GetContentShortTextCompatiblePadding(state),
                                                                      composition,
                                                                      glowing,
                                                                      fontChanged);
                                                  ...
                                                  bool glowing)
        {
            ...
            if ((longText != null) && (longText.Length > 0))
            {
                ...
                memento.LongTextMemento = AccurateText.MeasureString(g,
                                                                     ...
                                                                     paletteContent.GetContentLongTextCompatiblePadding(state),
                                                                     composition,
                                                                     glowing,
                                                                     fontChanged);
		...
		/// <returns></returns>
		protected virtual IDisposable DrawRibbonGroupAreaBorder2016(RenderContext context, Rectangle rect, PaletteState state, IPaletteRibbonBack palette, IDisposable memento)
		{
			if ((rect.Width > 0) && (rect.Height > 0))
			{
				Color c1 = palette.GetRibbonBackColor1(state);
				...
				bool generate = true;
				MementoRibbonGroupAreaBorder2016 cache;

				// Access a cache instance and decide if cache resources need generating
				if ((memento == null) || !(memento is MementoRibbonGroupAreaBorder2016))
				{
					if (memento != null)
						memento.Dispose();

					cache = new MementoRibbonGroupAreaBorder2016(rect, c1, c2);
					memento = cache;
				}
				else
				{
					cache = (MementoRibbonGroupAreaBorder2016)memento;
				...
				// Draw line at bottom
				context.Graphics.DrawLine(cache.borderPen, rect.X, rect.Bottom, rect.Right, rect.Bottom);
			}
