...
namespace ComponentFactory.Krypton.Toolkit
{
    ...
    #region Static Fields
        ...
        private static readonly Cursor _cursorHSplit = KryptonCursors.HSplitKryptonCursors.HSplit;//Properties.Resources.SplitHorizontal;
        private static readonly Cursor _cursorVSplit = KryptonCursors.HSplitKryptonCursors.VSplit;//Properties.Resources.SplitVertical;
    ...
    #region Implementation
        ...
        private void DrawSplitIndicator(Rectangle drawRect)
        {
            // We just perform a simple reversible rectangle draw
            //ControlPaint.FillReversibleRectangle(drawRect, Color.Black);

            var separator = _source.SeparatorControl;
            drawRect.Offset(separator.Location);
            if (separator.Parent != null)
                SplitIndicatorHelper.Instance.DrawReversibleSplitter(separator.Parent.Handle, drawRect);
        }
        ...
        private void UpdateCursor()
        {
            if (Cursor.Current == Cursors.Default)
            {
                ...
                if (_source.SeparatorOrientation == Orientation.Vertical)
                    Cursor.Current = (_splitCursors ? _cursorVSplit : _cursorVMove);
                else
                    Cursor.Current = (_splitCursors ? _cursorHSplit : _cursorHMove);
            }
        }
    #endregion
    }
