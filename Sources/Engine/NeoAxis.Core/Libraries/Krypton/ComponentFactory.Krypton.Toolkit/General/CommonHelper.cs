...
namespace ComponentFactory.Krypton.Toolkit
{
    ...
    public static class CommonHelper
    {
		...
		public static Padding GetWindowRealNCMargin(CreateParams cp)
        {
		...
		//{
		//	PI.RECT rect = new PI.RECT();
		//	Rectangle screen = ClientRectToScreen(f);
		//	rect.left = screen.Left;
		//	rect.top = screen.Top;
		//	rect.right = screen.Right;
		//	rect.bottom = screen.Bottom;
		//	bool flag = PI.AdjustWindowRectEx(ref rect, cp.Style, false, cp.ExStyle);
		//	Padding padding = Padding.Empty;
		//	if (flag)
		//		padding = new Padding(screen.Left - rect.left, screen.Top - rect.top, rect.right - screen.Right, rect.bottom - screen.Bottom);
		//	return padding;
		//}

		private static Rectangle ClientRectToScreen(Form f)
		{
			if (f.IsHandleCreated)
				return f.RectangleToScreen(f.ClientRectangle);
			Rectangle clientRectangle = f.ClientRectangle;
			clientRectangle.Offset(-f.Location.X, -f.Location.Y);
		...
		public static bool IsFormMinimized(Form f)
        {
			...
			int style = PI.GetWindowLong(f.Handle, PI.GWL_STYLE);
