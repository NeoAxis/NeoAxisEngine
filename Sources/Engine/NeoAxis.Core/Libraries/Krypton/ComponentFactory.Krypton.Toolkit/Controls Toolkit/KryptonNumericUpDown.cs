...
namespace Internal.ComponentFactory.Krypton.Toolkit
{
    ...
    //[Designer("ComponentFactory.Krypton.Toolkit.KryptonNumericUpDownDesigner, ComponentFactory.Krypton.Design, Version=125.0.0.0, Culture=neutral, PublicKeyToken=a87e673e9ecb6e8e")]
                                        ...
                                        IContainedInputControl
    {
                    ...
                    case PI.WM_PAINT:
                        {
                            ...
                            using (Graphics g = Graphics.FromHdc(hdc))
                            {
                                ...
                                else
                                {
                                    ...
                                    rect.left  -= (borderSize.Width + 2);
                                    rect.right += (borderSize.Width + 2) + 1;

                                    // Draw using a solid brush
                                    try
                                    {
                                        AccurateText.DrawString(g, _internalNumericUpDown.Text, _internalNumericUpDown.Font,
                                            new Rectangle(rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top), _internalNumericUpDown.ForeColor, stringFormat);
                                    }
                                    catch (ArgumentException)
                                    {
                                        AccurateText.DrawString(g, _internalNumericUpDown.Text, _kryptonNumericUpDown.GetTripleState().PaletteContent.GetContentShortTextFont(PaletteState.Disabled),
                                            new Rectangle(rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top), _internalNumericUpDown.ForeColor, stringFormat);
                                    }
                                }
                            ...
                            if (hBitmap != IntPtr.Zero)
                            {
                                try
                                {
                                    ...
                                    using (Graphics g = Graphics.FromHdc(_screenDC))
                                    {
                                        ...
                                        DrawUpDownButtons(g, new Rectangle(clientRect.X + 1, clientRect.Y, clientRect.Width - 1, clientRect.Height - 1));
        ...
        /// <summary>
        /// The value we take from the Text.
        /// 
        /// This hack is needed to control behaved correctly and does not reset the value 
        /// in the text field when working with Decimal values.
        /// </summary>
        [Browsable(false)]
        [Localizable(false)]
        [EditorBrowsable(EditorBrowsableState.Never)]
