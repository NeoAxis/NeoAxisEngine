...
namespace Internal.DotRecast.Detour.Extras.Jumplink
{
    ...
    {
        public DtJumpSegment[] Build(DtJumpLinkBuilderConfig acfg, DtEdgeSampler es)
        {
            int n = es.end[0].gsamples.Length;
            int[][] sampleGrid = RcArrays.Create2D<int>(n, es.end.Count);
            ...
            for (int j = 0; j < es.end.Count; j++)
            {
                for (int i = 0; i < n; i++)
                {
                    if (sampleGrid[i][j] == -1)
                    {
                        DtGroundSample p = es.end[j].gsamples[i];
            ...
            DtJumpSegment[] jumpSegments = new DtJumpSegment[region];
            for (int i = 0; i < jumpSegments.Length; i++)
            {
                jumpSegments[i] = new DtJumpSegment();
            }
        ...
        private void Fill(DtEdgeSampler es, int[][] sampleGrid, Queue<int[]> queue, float agentClimb, int region)
        {
        ...
        private void AddNeighbour(DtEdgeSampler es, Queue<int[]> queue, float agentClimb, float h, int i, int j)
        {
