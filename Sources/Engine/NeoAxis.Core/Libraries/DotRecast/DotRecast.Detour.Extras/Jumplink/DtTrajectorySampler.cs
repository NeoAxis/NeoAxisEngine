...
namespace Internal.DotRecast.Detour.Extras.Jumplink
{
    ...
    {
        public void Sample(DtJumpLinkBuilderConfig acfg, RcHeightfield heightfield, DtEdgeSampler es)
        {
            int nsamples = es.start.gsamples.Length;
            for (int i = 0; i < nsamples; ++i)
            {
                DtGroundSample ssmp = es.start.gsamples[i];
                foreach (DtGroundSegment end in es.end)
                {
                    DtGroundSample esmp = end.gsamples[i];
        ...
        private bool SampleTrajectory(DtJumpLinkBuilderConfig acfg, RcHeightfield solid, RcVec3f pa, RcVec3f pb, IDtTrajectory tra)
        {
