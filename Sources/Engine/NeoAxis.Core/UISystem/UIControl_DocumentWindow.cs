...
namespace NeoAxis.Editor
{
	public partial class UIControl_DocumentWindow : DocumentWindowWithViewport
	{
		...
		{
			if( controlFont == null )
				controlFont = ResourceManager.LoadResource<Component_Font>( @"Base\Fonts\FlowGraphEditor.ttf" );
		}
		...
		(UIControl, MoveModeEnum) GetMouseOverObject( bool onlyCanBeSelected )
		{
			...
			var resizeBorderSize = new Vector2( EditorAPI.DPI / 96.0 * 4.0, EditorAPI.DPI / 96.0 * 4.0 ) / viewport.SizeInPixels.ToVector2();
			...
			foreach( var control in GetControls( false ).GetReverse() )
			{
				...
				if( onlyCanBeSelected && !IsCanBeSelected( control ) )
					skip = true;
				if( dragDropObject != null && ( control == dragDropObject || control.GetAllParents( false ).Contains( dragDropObject ) ) )
					skip = true;
		...
		protected override void Viewport_MouseUp( Viewport viewport, EMouseButtons button, ref bool handled )
		{
			...
			if( button == EMouseButtons.Left )
			{
				...
				if( controlMove_Activated )
				{
					...
					{
						//cloned
						var action = new UndoActionComponentCreateDelete( Document, controlMove_Controls.ToArray(), true );
						Document.UndoSystem.CommitAction( action );
						Document.Modified = true;
		...
		protected override void Viewport_MouseMove( Viewport viewport, Vector2 mouse )//, ref bool handled )
		{
			...
			if( controlMove_Activated )
			{
				...
				foreach( var control in controlMove_Controls )
				{
					...
					{
						double marginSnap = ProjectSettings.Get.GetUIEditorStepMovement( startMargin.Measure );
						if( marginSnap != 0 )
						{
							var v2 = margin;
						...
						double sizeSnap = ProjectSettings.Get.GetUIEditorStepMovement( startSize.Measure );
						if( sizeSnap != 0 )
						{
							var v2 = size;
		...
		protected virtual void RenderBackground()
		{
			...
			//draw grid
			if( window.GetZoom() > .5f && ProjectSettings.Get.UIEditorDisplayGrid )
			{
		...
		ColorValue GetColorMultiplierSelectionState( EditorRenderSelectionState selectionState )
		{
		...
		protected override void Viewport_UpdateBeforeOutput( Viewport viewport )
		{
			...
			if( dragDropEntered && dragDropObject != null && dragDropObject is UIControl )
			{
				...
				if( parent != null )
				{
					...
					var color = ProjectSettings.Get.CanSelectColor;
			...
			//display selection of objects
			{
				...
				foreach( var control in GetControls( true ) )
				{
					...
					var selectionStateControl = EditorRenderSelectionState.None;
					...
					else
					{
						if( CanSelectObjects() )
						{
							if( mouseOverObject == control )
							{
								...
								ViewportControl.OneFrameChangeCursor = GetCursorByMoveMode( mouseOverObjectMoveMode );
							}
						...
						if( IsObjectSelected( control ) )
							selectionStateControl = EditorRenderSelectionState.Selected;
					}

					if( selectionStateControl != EditorRenderSelectionState.None )
					{
						...
						ColorValue color;
						if( selectionStateControl == EditorRenderSelectionState.Selected )
			...
			if( controlMove_Enabled )
				ViewportControl.OneFrameChangeCursor = GetCursorByMoveMode( controlMove_Mode );
		}
		...
		public void TryNewObject( Metadata.TypeInfo lockType )
		{
			...
			//{
			//	if( nodeCreationOnFlowchart && !typeof( Component_FlowchartNode ).IsAssignableFrom( selectedType.GetNetType() ) )
			//	{
			//		window.creationData.createdObjects = new List<object>();

			//		//create node with object inside of selected type
			//		var node = Control.CreateComponent<Component_FlowchartNode>( -1, false );
		...
		{
			var settingsAspectRatio = ProjectSettings.Get.UIEditorAspectRatio.Value;
		...
		{
			var size = ( ViewportControl.Viewport.SizeInPixels.ToVector2() * GetViewportRectangleInScreenCoords().Size ).ToVector2I();
			if( size.X < 1 )
				size.X = 1;
			if( size.Y < 1 )
				size.Y = 1;
		...
		void RenderTextureUpdate( float delta )
		{
			...
			if( renderTexture != null && ViewportControl.IsAllowRender() && !needRecreateDisplayObject )
			{
				var viewport = renderTexture.Result.GetRenderTarget().Viewports[ 0 ];
		...
		private void Component_UIControl_DocumentWindow_DragEnter( object sender, DragEventArgs e )
		{
		...
		private void Component_UIControl_DocumentWindow_DragOver( object sender, DragEventArgs e )
		{
		...
		private void Component_UIControl_DocumentWindow_DragLeave( object sender, EventArgs e )
		{
		...
		private void Component_UIControl_DocumentWindow_DragDrop( object sender, DragEventArgs e )
		{
		...
		void DragDropObjectCreate( DragEventArgs e )
		{
			...
			if( objectType != null && MetadataManager.GetTypeOfNetType( typeof( UIControl ) ).IsAssignableFrom( objectType ) )//|| memberFullSignature != "" || createNodeWithComponent != null )
			{
				//!!!!
				...
				var newObject = (UIControl)Control.CreateComponent( objectType );
		...
		void CalculateDropPosition( UIControl control, bool centered )
		{
			...
			if( ModifierKeys.HasFlag( Keys.Control ) )
			{
				var snap = ProjectSettings.Get.GetUIEditorStepMovement( UIMeasure.Units );
				if( snap != 0 )
				{
					...
					//if( destinationParent == Control )
					{
						...
						if( control != null )
						{
							if( n == 0 )
							{
								...
								//!!!!
								//addToPosition = control.Margin.Value.Value.LeftTop - ( (Component_FlowchartNode)c ).NodePosition;
							}
							else
							{
								//!!!!
		...
		public void Snap( EditorAction action )
		{
			...
			foreach( var obj in objects )
			{
				...
				var snapValue = ProjectSettings.Get.GetUIEditorStepMovement( newValue.Measure );
				if( snapValue != 0 )
				{
