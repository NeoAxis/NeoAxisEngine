...
namespace NeoAxis
{
	public partial class Scene
	{
		...
		void PhysicsWorldCreate()
		{
			...
			//!!!!
			//!!!!need update sources of bullet and wrapper
			var testMultiThreaded = false;// true;
			...
			//!!!!no multithreaded implementation
			...
			data.softBodyWorldInfo.WaterNormal = Internal.BulletSharp.Math.BVector3.Zero;
		...
		internal unsafe void PhysicsSimulate( bool editorSimulateSelectedMode, ESet<IPhysicalObject> updateOnlySelected )
		{
			...
			//get updated data
			{
				...
				for( int n = 0; n < updateObjectsCount; n++ )
				{
					...
					{
						var body = collisionObject.UserObject as PhysicalBody;
				...
				for( int n = 0; n < constraintCount; n++ )
				{
					...
					if( c2 != null && ( updateOnlySelected == null || updateOnlySelected.Contains( c2 ) ) )
						c2.UpdateDataFromPhysicsEngine();
				}

				//foreach( var body in GetComponents<PhysicalBody>( false, true, true ) )
				//	body.UpdateDataFromPhysicsEngine();

				//foreach( var constraint in GetComponents<Constraint>( false, true, true ) )
				//	constraint.UpdateDataFromPhysicsEngine();
			}

			//clear old collision contacts
			if( !editorSimulateSelectedMode )
			{
				//!!!!slowly

				foreach( var collisionObject in physicsWorldData.world.CollisionObjectArray )
				{
					var body = collisionObject.UserObject as RigidBody;
			...
			//process collision contacts
			{
				...
				if( sizeof( IntPtr ) == 8 )
				{
					if( sizeof( CollectContactsItem ) != 248 )
						Log.Fatal( "sizeof( CollectContactsItem ) != 248. Size: " + size.ToString() );
				}
				else
				{
					if( sizeof( CollectContactsItem ) != 240 )
						Log.Fatal( "sizeof( CollectContactsItem ) != 240. Size: " + size.ToString() );
				}
				...
				if( !editorSimulateSelectedMode )
				{
					for( int nContact = 0; nContact < contactsCount; nContact++ )
					{
						...
						{
							var bodyA = collisionObjectA.UserObject as RigidBody;
							var bodyB = collisionObjectB.UserObject as RigidBody;
							//var bodyA = collisionObjectA.UserObject as PhysicalBody;
							//var bodyB = collisionObjectB.UserObject as PhysicalBody;
							if( bodyA != null && !bodyA.Disposed && bodyB != null && !bodyB.Disposed )
							{
								if( bodyA.ContactsCollect )
								{
									...
									{
										var item = new RigidBody.ContactsDataItem();
										...
										if( bodyA.ContactsData == null )
											bodyA.ContactsData = new OpenList<RigidBody.ContactsDataItem>();
										if( bodyA.ContactsData.Count < 128 )
											bodyA.ContactsData.Add( ref item );
									}
								}
								...
								if( bodyB.ContactsCollect )
								{
									for( int n = 0; n < contactsItem->numContacts; n++ )
									{
										...
										if( bodyB.ContactsData == null )
											bodyB.ContactsData = new OpenList<RigidBody.ContactsDataItem>();
										if( bodyB.ContactsData.Count < 128 )
											bodyB.ContactsData.Add( ref item );
									}
								}
							}
						}
				...
				//	var rigidBodyA = obA.UserObject as RigidBody;
				//	var rigidBodyB = obB.UserObject as RigidBody;
				...
				//						rigidBodyA.ContactsData = new List<RigidBody.ContactsDataItem>();
				...
				//						rigidBodyB.ContactsData = new List<RigidBody.ContactsDataItem>();
		...
		class BulletConvexResultCallback : ConvexResultCallback, IComparer<PhysicsConvexSweepTestItem.ResultItem>
		{
			...
			PhysicsConvexSweepTestItem.ResultItem CreateResultItem( CollisionObject obj, double hitFraction )
			{
				...
				resultItem.Body = obj.UserObject as ObjectInSpace;
				resultItem.Normal = BulletPhysicsUtility.Convert( hitNormalWorld );
				resultItem.DistanceScale = hitFraction;
