...
namespace NeoAxis
{
	partial class Component_Mesh
	{
		...
		class GeometryEntry
		{
			...
			public static GeometryEntry ToGeometryEntry( Component_MeshGeometry g )
			{
				if( g is Component_MeshGeometry_Procedural )
					return null;
		...
		/// 
		/// </summary>
		/// <param name="mergeTriangles"> If true. Merges adjacent triangles if their normals ar equal</param>
		public void RebuildStructure( bool mergeTriangles = true, float normalComparisonEpsilon = 1e-4f )
		{
			//geometries[x] == null for Component_MeshGeometry_Procedural
			//Procedural geometries are skipped because their structures are appended in ExtractStructure()
			GeometryEntry[] geometries = GetComponents<Component_MeshGeometry>().Select( GeometryEntry.ToGeometryEntry ).ToArray();
