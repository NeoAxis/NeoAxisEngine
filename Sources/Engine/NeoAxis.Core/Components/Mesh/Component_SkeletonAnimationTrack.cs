...
namespace NeoAxis
{
	...
	/// </summary>
	public class Component_SkeletonAnimationTrack : Component
	{
		//KeyFrames must be groupped by BoneIndex, and inside the group must be ordered by time. The bone groups ordered like Component_Skeleton.GetBones().
		/// <summary>
		/// Data of the track.
		/// </summary>
		[Serialize]
		...
		public event Action<Component_SkeletonAnimationTrack> KeyFramesChanged;
		ReferenceField<byte[]> _keyFrames;

		/////////////////////////////////////////

		/// <summary>
		/// Represents key frame data of <see cref="Component_SkeletonAnimationTrack"/>.
		/// </summary>
		...
		public unsafe void CalculateBoneTransforms( double time, CalculateBoneTransformsItem[] result )
		{
			...
			if( keyFrames2.Length % sizeof( KeyFrame ) != 0 )
				Log.Fatal( "Component_SkeletonAnimationTrack: CalculateBoneTransforms: keyFrames2.Length % sizeof( KeyFrame ) != 0." );
