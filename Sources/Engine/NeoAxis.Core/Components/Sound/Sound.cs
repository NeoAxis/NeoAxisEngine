...
namespace NeoAxis
{
	//!!!!можно еще .settings дл€ того, чтобы добавить опции. также как импорт или еще что-то
	...
	public class Sound : ResultCompile<Sound.CompiledData>
	{
		...
		public class CompiledData : IDisposable
		{
			...
			{
				//!!!!threading: как будет, если из другого потока еще юзаем, но меш обновилс€
		...
		protected override void OnResultCompile()
		{
			if( Result == null )
			{
				...
				//else
				{
					...
					//каждый раз провер€ть будет если нет файла. где еще так?
