...
namespace NeoAxis
{
	...
	public class RenderingEffect_Reflection : RenderingEffect
	{
		...
		//bool GetShowScreenSpaceProperties()
		//{
		//}
		...
		protected override void OnMetadataGetMembersFilter( Metadata.GetMembersContext context, Metadata.Member member, ref bool skip )
		{
			...
			//!!!!какие их свойств для full mode тоже нужны
		...
		{
			var multiplier = Math.Min( GlobalMultiplier, 1 ) * Intensity;

			if( EnabledInHierarchy && sender.GetUseMultiRenderTargets() && multiplier > 0 /*Intensity > 0*/ )
			{
				...
				var reflectionTextureSize = ( actualTexture.Result.ResultSize.ToVector2() * Math.Max( GlobalMultiplier, 1 ) / (int)Resolution.Value ).ToVector2I();
				...
				var finalTexture = context.RenderTarget2D_Alloc( actualTexture.Result.ResultSize, actualTexture.Result.ResultFormat );
				{
					...
					shader.Parameters.Set( "invViewProj", invViewProjMatrix );
					shader.Parameters.Set( "cameraPosition", cameraPosition );
					shader.Parameters.Set( "intensity", (float)multiplier );// Intensity );
