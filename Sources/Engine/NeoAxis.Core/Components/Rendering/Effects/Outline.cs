...
namespace NeoAxis
{
	...
	public class RenderingEffect_Outline : RenderingEffect
	{
		...
		void ProcessEffect( ViewportRenderingContext context, RenderingPipeline_Basic.FrameData frameData, ref ImageComponent actualTexture )
		{
			...
			//sort by group index
			var sortedIndices = new int[ items2.Count ];
			for( int n = 0; n < sortedIndices.Length; n++ )
				sortedIndices[ n ] = n;

			CollectionUtility.MergeSort( sortedIndices, delegate ( int indexA, int indexB )
			{
				var a = items2[ indexA ];
				var b = items2[ indexB ];
				if( a.Item1 < b.Item1 )
			...
			for( int n = 0; n < sortedIndices.Length; n++ )
			{
				var item = items2[ sortedIndices[ n ] ];
