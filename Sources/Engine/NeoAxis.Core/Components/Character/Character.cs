	...
	public class Character : MeshInSpace, IProcessDamage, InteractiveObjectInterface, MeshInSpaceAnimationController.IParentAnimationTriggerProcess
	{
		...
		public bool Sitting
		{
			...
			set
			{
				...
				if( NetworkIsServer )
				{
					var m = BeginNetworkMessageToEveryone( "Sitting" );
					m.Writer.Write( Sitting );
					m.End();
				}
			}
		}
		...
		public Vector3? CurrentLookToPosition
		{
			...
			set
			{
				...
				if( NetworkIsServer )
				{
					...
					var m = BeginNetworkMessageToEveryone( "CurrentLookToPosition" );
					m.Writer.Write( currentLookToPosition.HasValue );
					if( currentLookToPosition.HasValue )
						m.Writer.Write( currentLookToPosition.Value );
					m.End();
				}
			}
		}
		...
		protected override void OnSimulationStep()
		{
			...
			if( NetworkIsServer )
			{
				if( sentToClientsIsOnGround != IsOnGround() || sentToClientsGroundBody1Velocity != groundBody1Velocity )
				{
					...
					var m = BeginNetworkMessageToEveryone( "OnGroundState" );
					if( m != null )
					{
						m.Writer.Write( sentToClientsIsOnGround );
						m.Writer.Write( sentToClientsGroundBody1Velocity );
						m.End();
					}
				}
				...
				if( sentToClientsSmoothCameraOffsetZ != smoothCameraOffsetZ )
				{
					...
					var m = BeginNetworkMessageToEveryone( "SmoothCameraOffsetZ" );
					if( m != null )
					{
						m.Writer.Write( sentToClientsSmoothCameraOffsetZ );
						m.End();
					}
				}
			}
			...
			//if( TypeCached.Animate )
			{
				...
				if( controller != null )
				{
					...
					if( activeItem != null )
					{
						...
						basicItemAnimations = activeItem as Item3D != null;
					}
					else
					{
						activeWeapon = null;
		...
		protected virtual void AdditionalBoneTransformsUpdate( MeshInSpaceAnimationController controller, MeshInSpaceAnimationController.AnimationStateClass animationState, Skeleton skeleton, SkeletonAnimationTrack.CalculateBoneTransformsItem[] outputBoneTransforms )
		{
			...
			var activeItem = GetActiveItem();
			Weapon weapon = null;
			Item3D basicItem = null;
			if( activeItem != null )
			{
				weapon = activeItem as Weapon;
				basicItem = activeItem as Item3D;
			}
		...
		public bool ItemDrop( GameMode gameMode, Item3DInterface item/*, bool calculateTransform, Transform setTransform*/, double amount )
		{
			...
			else
			{
				...
				( (Item3D)item2 ).ItemCount = amount2;
			}
		...
		public void ItemDropClient( Item3DInterface item, double amount )
		{
			...
			if( component != null )
			{
				...
				if( m != null )
				{
					...
					m.Writer.Write( amount );//writer.WriteVariableInt32( amount );
					m.End();
				}
			}
		}
				...
				if( LifeStatus.Value == LifeStatusEnum.Normal )
				{
					...
					if( health > 0 )
					{
						...
						if( Health.Value <= 0 )
						{
							...
							if( NetworkIsServer )
							{
								var m = BeginNetworkMessageToEveryone( "PlayDieAnimation" );
								m.End();
							}
		...
		{
			if( trigger.AnyDataTrigger == "Footstep" && animationItem.Factor > 0.5 )
			{
				var handled = false;
