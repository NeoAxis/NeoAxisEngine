...
namespace NeoAxis
{
	...
	/// </summary>
	public class FlowSetVariable : Component, IFlowGraphRepresentationData, IFlowExecutionComponent
	{
		...
		public event Action<FlowSetVariable> ExitChanged;

		//Variable
		ReferenceField<FlowDeclareVariable> _variable;
		/// <summary>
		/// The variable whose value needs to be set.
		/// </summary>
		[Serialize]
		public Reference<FlowDeclareVariable> Variable
		{
			get
			{
			...
			set
			{
				if( _variable.BeginSet( this, ref value ) )
				{
					try
					{
		...
		public event Action<FlowSetVariable> VariableChanged;
