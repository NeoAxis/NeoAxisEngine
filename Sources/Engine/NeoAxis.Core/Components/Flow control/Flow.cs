...
namespace NeoAxis
{
	...
	{
		void FlowExecution( Flow flow, Flow.ExecutionStackItem entryItem, ref bool sleep );
	}
	...
	public class Flow
	{
		...
		internal static double globalSleepingFlowsTimeDelta;

		//

		ComponentHierarchyController owner;
		...
		public static Flow ExecuteWithoutRemoveFromStack( ComponentHierarchyController owner, FlowInput entry, IEnumerable<Tuple<string, object>> initVariables = null )
		{
			InitFlowStack();
		...
		public ComponentHierarchyController Owner
		{
			get { return owner; }
		}
		...
		//!!!!for SetVariable
		public static void SetVariableToCurrentFlow( FlowDeclareVariable variable, object value )
		{
			if( CurrentFlow != null )
		...
		internal static void ExecuteGlobalSleepingFlows()
		{
			Flow[] flows;
			lock( globalSleepingFlows )
			{
				flows = globalSleepingFlows.ToArray();
				globalSleepingFlows.Clear();
			}

			globalSleepingFlowsTimeDelta = EngineApp.EngineTime - globalSleepingFlowsLastEngineTime;
			if( globalSleepingFlowsLastEngineTime == 0 )
				globalSleepingFlowsTimeDelta = 0.001;
			globalSleepingFlowsLastEngineTime = EngineApp.EngineTime;
