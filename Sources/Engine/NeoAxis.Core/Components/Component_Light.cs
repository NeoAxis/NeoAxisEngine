...
{
	////!!!!good?
	//public interface IComponent_LightChild
	//{
	//	void RenderUI( Component_RenderingPipeline.RenderSceneData.LightItem lightItem, Viewport viewport );
	//}

	//!!!!разделить может всё же на два класса: тип и инстанс?
	//!!!!у источника могут быть какие-то большие данные. какие-то нибудь гармоники или трафареты. т.е. как мещ типизировать

	/// <summary>
	/// The light source in the scene.
	/// </summary>
	public class Component_Light : Component_ObjectInSpace//Component_ResultCompile<Component_LightSource.ResultDataClass>
	{
		...
		public event Action<Component_Light> TypeChanged;
		...
		//!!!!max value
		[Range( 0, 1000000, RangeAttribute.ConvenientDistributionEnum.Exponential, 5 )]
		...
		public event Action<Component_Light> BrightnessChanged;
		...
		public event Action<Component_Light> ColorChanged;
		...
		//}
		//public event Action<Component_Light> PowerChanged;
		...
		public event Action<Component_Light> AttenuationNearChanged;
		...
		public event Action<Component_Light> AttenuationFarChanged;
		...
		public event Action<Component_Light> AttenuationPowerChanged;
		...
		public event Action<Component_Light> SpotlightInnerAngleChanged;
		...
		public event Action<Component_Light> SpotlightOuterAngleChanged;
		...
		public event Action<Component_Light> SpotlightFalloffChanged;
		...
		public event Action<Component_Light> ShadowsChanged;
		...
		public event Action<Component_Light> ShadowIntensityChanged;
		...
		//}
		//public event Action<Component_Light> ShadowSoftnessChanged;
		//ReferenceField<double> _shadowSoftness = 1.0;

		//!!!!default value. хотя 0.5 - это более стандартно, чем какой-нибудь 0.02
		/// <summary>
		...
		public event Action<Component_Light> ShadowBiasChanged;
		...
		public event Action<Component_Light> ShadowNormalBiasChanged;
		...
		/// </summary>
		[DefaultValue( null )]
		public Reference<Component_Image> Mask
		{
		...
		public event Action<Component_Light> MaskChanged;
		ReferenceField<Component_Image> _mask;
		...
		public event Action<Component_Light> MaskScaleChanged;
		...
		//}
		//public event Action<Component_Light> MaskPositionChanged;
		...
		//}
		//public event Action<Component_Light> MaskMatrixChanged;
		...
		public override void OnGetRenderSceneData( ViewportRenderingContext context, GetRenderSceneDataMode mode, Component_Scene.GetObjectsInSpaceItem modeGetObjectsItem )
		{
			if( EnabledInHierarchy && VisibleInHierarchy && mode == GetRenderSceneDataMode.InsideFrustum )
			{
				...
				{
					var item = new Component_RenderingPipeline.RenderSceneData.LightItem();
					item.Creator = this;
					...
					if( Components.Count != 0 )
					{
						...
						//item.children = GetComponents<IComponent_LightChild>( onlyEnabledInHierarchy: true );
					}
