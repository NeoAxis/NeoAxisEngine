...
namespace NeoAxis
{
	...
	//[EditorSettingsCell( typeof( ShaderScript_SettingsCell ) )]
	[EditorControl( "NeoAxis.Editor.ShaderScriptEditor" )]
	//[EditorPreviewControl( typeof( ShaderScript_PreviewControl ) )]
	public class ShaderScript : Component//!!!!, IFlowGraphRepresentationData, IFlowExecutionComponent
	{
		...
		//!!!!
		////FlowGraph
		//Image cachedFlowGraphNodeImage;
		...
		ReferenceField<TypeEnum> _type = TypeEnum.Function;
...
";
		...
		public event Action<ShaderScript> CodeChanged;
		...
		//	}
		//}
		//public event Action<ShaderScript> FlowSupportChanged;
		...
		/// <summary>
		/// Represents a compiled data of <see cref="ShaderScript"/>.
		/// </summary>
		...
		static CompiledShaderScript GetOrCompileScript( string code, out string error )
		{
			...
			{
				if( line2.Contains( "public" ) )//if( line2.Contains( @"/*!public*/" ) )
				{
					line = line2;
					break;
				}
			...
			try
			{
				...
				if( line.Contains( @"/*!public*/" ) )
					str1 = line.Substring( line.IndexOf( @"/*!public*/" ) + @"/*!public*/".Length );
				else
					str1 = line.Substring( line.IndexOf( "public" ) + "public".Length );
		...
		void Update()
		{
			...
			//check disabled
			if( !Enabled || Type.Value != TypeEnum.Function )
			{
				Clear();
				return;
			}
