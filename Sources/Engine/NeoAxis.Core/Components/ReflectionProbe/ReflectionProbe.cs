...
namespace NeoAxis
{
	...
	public class ReflectionProbe : ObjectInSpace
	{
		...
		//!!!!может зависеть от размера экрана. дл€ realtime
		/// <summary>
		...
		void UpdateProcessedCubemaps()
		{
			...
			if( processedCubemapNeedUpdate && !RenderingSystem.BackendNull )//&& AllowProcessEnvironmentCubemap )
			{
		...
		//!!!!вызывать когда не нужен. когда не показываетс€
