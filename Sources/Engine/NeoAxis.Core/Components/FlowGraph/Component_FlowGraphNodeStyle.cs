...
namespace NeoAxis
{
	...
	/// </summary>
	public abstract class Component_FlowGraphNodeStyle : Component
	{
		...
		public abstract void RenderNode( Component_FlowGraph_DocumentWindow window, Component_FlowGraphNode node,
		...
		public abstract object GetMouseOverObject( Component_FlowGraph_DocumentWindow window, Component_FlowGraphNode node );

		public abstract bool IsIntersectsWithRectangle( Component_FlowGraph_DocumentWindow window, Component_FlowGraphNode node, Rectangle rectInUnits );
	}

	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	//!!!!
	//public class Component_FlowchartNodeStyle_Circle : Component_FlowchartNodeStyle
	//{
	//}
	...
	/// </summary>
	public class Component_FlowGraphNodeStyle_Rectangle : Component_FlowGraphNodeStyle
	{
		static Component_FlowGraphNodeStyle_Rectangle instance;
		/// <summary>
		/// Singleton object of the style.
		/// </summary>
		public static Component_FlowGraphNodeStyle_Rectangle Instance
		{
			get
			{
				if( instance == null )
				{
					instance = new Component_FlowGraphNodeStyle_Rectangle();
			...
			if( window.GetZoom() > .25f )
			{
				...
				for( int nItem = 0; nItem < representation.Items.Count; nItem++ )
				{
					...
					if( item.Input != null )
					{
						...
						var texture = ResourceManager.LoadResource<Component_Image>( textureName );
					...
					if( item.Output != null )
					{
						...
						if( referenceCreationSocketFrom != null && window.CanCreateReference( item.Output, referenceCreationSocketFrom ) )
							circleColor = circleColorCanConnect;
						if( !isFlow && dragDropSetReferenceData != null && window.CanCreateReferenceDragDropSetReference( dragDropSetReferenceData, item.Output, out string[] dummy ) )
						{
							circleColor = circleColorCanConnect;
						}
		...
		public override object GetMouseOverObject( Component_FlowGraph_DocumentWindow window, Component_FlowGraphNode node )
		{
			var representation = node.GetRepresentation();
			var viewport = window.ViewportControl.Viewport;
			var mouse = viewport.MousePosition;
				...
				for( int nSocket = 0; nSocket < representation.Items.Count; nSocket++ )
				{
					...
					if( socket.Input != null )
					{
						...
						if( rect.Contains( mouse ) )
							return new Component_FlowGraphNode.Representation.Connector( socket, true );
					}
					...
					if( socket.Output != null )
					{
						...
						Rectangle rect = window.ConvertUnitToScreen( new Rectangle( center - size / 2, center + size / 2 ) );
						if( rect.Contains( mouse ) )
							return new Component_FlowGraphNode.Representation.Connector( socket, false );
					}
				}
			}
		...
		public override bool IsIntersectsWithRectangle( Component_FlowGraph_DocumentWindow window, Component_FlowGraphNode node, Rectangle rectInUnits )
		{
			//!!!!slowly?
