...
namespace NeoAxis.Import.FBX
{
	...
	{
		public static void CalculateIndicesAndMergeEqualVertices( MeshData data, out StandardVertex[] vertices, out int[] indices )
		{
			var bounds = BoundsF.Cleared;
			for( int n = 0; n < data.Vertices.Length; n++ )
			{
				ref var v = ref data.Vertices[ n ];
				bounds.Add( ref v.Vertex.Position );
			}

			var maxBounds = bounds.GetSize().MaxComponent();

			float vertexEpsilon = maxBounds / 10000.0f;

			//float positionEpsilon = data.CalcCache.PositionEpsilon;

			//vertices = new StandardVertex[ data.Vertices.Length ];
			//for( int n = 0; n < vertices.Length; n++ )
			//	vertices[ n ] = data.Vertices[ n ].Vertex;

			//indices = new int[ vertices.Length ];
			//for( int n = 0; n < indices.Length; n++ )
			//	indices[ n ] = n;

			//MathAlgorithms.MergeEqualVertices( ref vertices, ref indices, positionEpsilon );//, true );

			var sourceVertices = new StandardVertex[ data.Vertices.Length ];
			for( int n = 0; n < sourceVertices.Length; n++ )
				sourceVertices[ n ] = data.Vertices[ n ].Vertex;

			var sourceIndices = new int[ sourceVertices.Length ];
			for( int n = 0; n < sourceIndices.Length; n++ )
				sourceIndices[ n ] = n;

			MathAlgorithms.MergeEqualVerticesRemoveInvalidTriangles( sourceVertices, sourceIndices, 0, vertexEpsilon, out vertices, out indices, out _ );
		}
