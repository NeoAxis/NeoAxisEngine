...
namespace NeoAxis
{
	...
	/// Rigid physical body.
	/// </summary>
	public class Component_RigidBody : Component_PhysicalBody
	{
		...
		public struct ContactsDataItem
		{
			...
			public Component_RigidBody BodyB;
		}
		List<ContactsDataItem> contactsData;
		...
		public event Action<Component_RigidBody> MotionTypeChanged;
		...
		public event Action<Component_RigidBody> MassChanged;
		...
		public event Action<Component_RigidBody> EnableGravityChanged;
		...
		public event Action<Component_RigidBody> LinearDampingChanged;
		...
		public event Action<Component_RigidBody> MaterialChanged;
		...
		[Serialize]
		[Category( "Rigid Body" )]
		public Reference<Component_PhysicalMaterial.FrictionModeEnum> MaterialFrictionMode
		{
		...
		public event Action<Component_RigidBody> MaterialFrictionModeChanged;
		...
		public event Action<Component_RigidBody> MaterialFrictionChanged;
		...
		public event Action<Component_RigidBody> MaterialAnisotropicFrictionChanged;
		...
		public event Action<Component_RigidBody> MaterialSpinningFrictionChanged;
		...
		public event Action<Component_RigidBody> MaterialRollingFrictionChanged;
		...
		public event Action<Component_RigidBody> MaterialRestitutionChanged;
		...
		public event Action<Component_RigidBody> CcdChanged;
		...
		public event Action<Component_RigidBody> CcdSweptSphereRadiusChanged;
		...
		public event Action<Component_RigidBody> CcdMotionThresholdChanged;
		...
		public event Action<Component_RigidBody> CenterOfMassManualChanged;
		...
		public event Action<Component_RigidBody> CenterOfMassPositionChanged;
		...
		public event Action<Component_RigidBody> InertiaTensorFactorChanged;
		...
		public event Action<Component_RigidBody> LinearSleepingThresholdChanged;
		...
		public event Action<Component_RigidBody> AngularSleepingThresholdChanged;
		...
		public event Action<Component_RigidBody> CollisionGroupChanged;
		...
		public event Action<Component_RigidBody> CollisionMaskChanged;
		...
		public event Action<Component_RigidBody> LinearVelocityChanged;
		...
		public event Action<Component_RigidBody> AngularVelocityChanged;
		...
		public event Action<Component_RigidBody> ContactsCollectChanged;
		...
		public event Action<Component_RigidBody> ContactsDisplayChanged;
		...
		//	}
		//}
		//public event Action<Component_RigidBody> CanCreateChanged;
		...
		protected override void OnMetadataGetMembersFilter( Metadata.GetMembersContext context, Metadata.Member member, ref bool skip )
		{
			...
			if( p != null )
			{
				...
				case nameof( MaterialAnisotropicFriction ):
					if( MaterialFrictionMode.Value == Component_PhysicalMaterial.FrictionModeEnum.Simple || Material.Value != null )
						skip = true;
					break;
		...
		static void GetComponentShapesRecursive( Component_CollisionShape shape, Matrix4 shapeTransform, List<Tuple<Component_CollisionShape, Matrix4>> result )
		{
			result.Add( new Tuple<Component_CollisionShape, Matrix4>( shape, shapeTransform ) );

			foreach( var child in shape.GetComponents<Component_CollisionShape>( false, false, true ) )
			{
		...
		protected override void OnSpaceBoundsUpdate( ref SpaceBounds newBounds )
		{
			...
			if( rigidBody != null )
			{
				rigidBody.GetAabb( out BulletSharp.Math.Vector3 min, out BulletSharp.Math.Vector3 max );
		...
		bool CanCreate
		{
			get
			{
				if( Name == "Collision Definition" && Parent as Component_Mesh != null )
					return false;
				return true;
			}
		}
		...
		void CreateBody()
		{
			...
			{
				if( rigidBody != null )
					Log.Fatal( "Component_RigidBody: CreateBody: rigidBody != null." );
				if( !EnabledInHierarchy )
					Log.Fatal( "Component_RigidBody: CreateBody: !EnabledInHierarchy." );
				...
				var componentShapes = new List<Tuple<Component_CollisionShape, Matrix4>>();
				foreach( var child in GetComponents<Component_CollisionShape>( false, false, true ) )
				...
				if( componentShapes.Count > 0 )
				{
					...
					if( shape != null )
					{
						...
						if( MotionType.Value == MotionTypeEnum.Dynamic )
						{
							...
							BulletSharp.Math.Vector3 localInertia = shape.CalculateLocalInertia( mass ) * BulletPhysicsUtility.Convert( InertiaTensorFactor.Value );
		...
		List<Component_Constraint> GetLinkedCreatedConstraints()
		{
			var list = new List<Component_Constraint>( rigidBody.NumConstraintRefs );
		...
		static AnisotropicFrictionFlags ConvertFrictionMode( Component_PhysicalMaterial.FrictionModeEnum value )
		{
			switch( value )
			{
			case Component_PhysicalMaterial.FrictionModeEnum.Simple:
				return AnisotropicFrictionFlags.FrictionDisabled;
			case Component_PhysicalMaterial.FrictionModeEnum.Anisotropic:
				return AnisotropicFrictionFlags.Friction;
			case Component_PhysicalMaterial.FrictionModeEnum.AnisotropicRolling:
				return AnisotropicFrictionFlags.RollingFriction;
			}
			return 0;
		}

		void SetMaterial( RigidBody b )
		{
			//material settings
			Component_PhysicalMaterial mat = Material;
			if( mat != null )
			{
				var mode = mat.FrictionMode.Value;

				if( mode == Component_PhysicalMaterial.FrictionModeEnum.Simple )
				{
					b.SetAnisotropicFriction( BulletSharp.Math.Vector3.One, AnisotropicFrictionFlags.FrictionDisabled );
		...
		public override void Render( ViewportRenderingContext context, out int verticesRendered )
		{
			...
			if( /*show && */rigidBody != null && context.Owner.Simple3DRenderer != null )
			{
				...
				//draw body
				{
					...
					//!!!!checkChildren?
					foreach( var shape in GetComponents<Component_CollisionShape>( false, true, true ) )
