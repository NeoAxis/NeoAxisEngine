...
namespace NeoAxis
{
	...
	public class Component_Constraint : Component_ObjectInSpace, Component_IPhysicalObject
	{
		...
		void GetSoftBodyModeBodies( out IComponent_SoftBody body1, out Component_PhysicalBody body2 )
		{
			//body1 is always soft body
			if( creationBodyA != null && creationBodyA.BulletBody is SoftBody )
			{
				body1 = (IComponent_SoftBody)creationBodyA;
				body2 = creationBodyB;
			}
			else
			{
				body1 = (IComponent_SoftBody)creationBodyB;
				body2 = creationBodyA;
			}
		}
			...
			if( SoftBodyMode )
			{
				...
				else
				{
					...
					if( AllLinear( PhysicsAxisMode.Free ) )
					{
						...
						if( body2 == null )
						{
							context.Owner.Simple3DRenderer.AddLine( trans.Position, ( (Component_ObjectInSpace)body1 ).Transform.Value.Position );
							verticesRendered += 8;
						}
						else
						{
							context.Owner.Simple3DRenderer.AddLine( trans.Position, ( (Component_ObjectInSpace)body1 ).Transform.Value.Position );
							context.Owner.Simple3DRenderer.AddLine( trans.Position, ( (Component_ObjectInSpace)body2 ).Transform.Value.Position );
							verticesRendered += 16;
						}
					}
		...
		void DrawLinearLimitedJoint( RenderingContext context, Vector3 p1, Vector3 p2, IComponent_SoftBody body1, Component_PhysicalBody body2, ref int verticesRendered )
		{
			context.viewport.Simple3DRenderer.SetColor( new ColorValue( 0, 0, 0 ) );

			context.viewport.Simple3DRenderer.AddLine( p1, ( (Component_ObjectInSpace)body1 ).Transform.Value.Position );
			context.viewport.Simple3DRenderer.AddLine( p2, ( (Component_ObjectInSpace)body1 ).Transform.Value.Position );
			verticesRendered += 16;
