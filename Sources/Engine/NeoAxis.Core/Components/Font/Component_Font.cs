...
namespace NeoAxis
{
	...
	public class Component_Font : Component
	{
		...
		static Net3dBool.Vector3[] Convert( IList<Vector3F> source )
		{
			var result = new List<Net3dBool.Vector3>( source.Count );
			foreach( var v in source )
				result.Add( new Net3dBool.Vector3( v.X, v.Y, v.Z ) );
			return result.ToArray();
		}

		static List<Vector3F> Convert( Net3dBool.Vector3[] source )
		{
			...
			public void GetShape( out List<Vector3F> resultPositions, out List<Vector3F> resultNormals, out List<Vector4F> resultTangents, out List<int> resultIndices )
			{
				if( characterPositions == null )
				{
					...
					foreach( var c in Remove )
					{
						...
						var operand2 = new Net3dBool.Solid( Convert( positions2 ), indices2.ToArray() );
						var modeller = new Net3dBool.BooleanModeller( new Net3dBool.Solid( Convert( characterPositions ), characterIndices.ToArray() ), operand2 );
