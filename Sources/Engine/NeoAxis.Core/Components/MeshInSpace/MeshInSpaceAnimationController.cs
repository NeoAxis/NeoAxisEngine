...
namespace NeoAxis
{
	...
	public class MeshInSpaceAnimationController : Component//, MeshInSpace.IMeshInSpaceChild//MeshInSpaceController
	{
		...
		//					//{
		//					//	item.AnimationData = new RenderingPipeline.RenderSceneData.MeshItem.AnimationDataClass();
		...
		//					//	var bonesTexture = context.DynamicTexture_Alloc( ViewportRenderingContext.DynamicTextureType.DynamicTexture, Image.TypeEnum._2D, size, PixelFormat.Float32RGBA, 0, false );
		...
		void UpdateAnimationTime()
		{
			...
			//var animation = PlayAnimation.Value as SkeletonAnimation;
			//if( animation != null )
			//{
		...
		void ResetSkeletonAndAnimation( bool needResetToOriginalMesh )
		{
			...
			//currentAnimationTime = ( PlayAnimation.Value as SkeletonAnimation )?.TrackStartTime ?? 0;
					...
					if( animations.Count != 0 )
					{
						...
						for( int nIteration = 0; nIteration < ( updateTwice ? 2 : 1 ); nIteration++ )
						{
							if( animations.Count > 1 )
							{
								...
								var transforms = new SkeletonAnimationTrack.CalculateBoneTransformsItem[ boneTransforms.Length ];
								var count = 0;
