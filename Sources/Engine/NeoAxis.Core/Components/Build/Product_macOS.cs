...
namespace NeoAxis
{
	...
	/// </summary>
	public class Product_macOS : Product
	{
		/// <summary>
		...
		{
			get { return SystemSettings.Platform.macOS; }
		}
		...
		protected override void OnGetPaths( List<string> paths )
		{
			...
			//exclude from Binaries
			{
				...
				//!!!!
				//excludePaths.Add( Path.Combine( sourceFolder, @"NeoAxis.Internal\Platforms\macOS\dotnet" ) );
				...
				if( !UIWebBrowser )
					excludePaths.Add( Path.Combine( sourceFolder, @"NeoAxis.Internal\Platforms\macOS\CefGlue" ) );
