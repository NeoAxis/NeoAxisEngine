...
namespace NeoAxis
{
	...
	/// </summary>
	public class Product_iOS : Product
	{
		//!!!!всё тут
		...
		//public event Action<Product_iOS> BuildPlatformChanged;
		...
		public event Action<Product_iOS> CompressDataChanged;
		...
		//public event Action<Product_iOS> PatchProjectFilesChanged;
		...
		//public event Action<Product_iOS> PackageDisplayNameChanged;
		...
		//public event Action<Product_iOS> PackageVersionChanged;
		...
		//public event Action<Product_iOS> PackagePublisherChanged;
		...
		//public event Action<Product_iOS> PublisherDisplayNameChanged;
		...
		////public event Action<Product_iOS> CreateAppxPackageChanged;
		...
		//public event Action<Product_iOS> AppDisplayNameChanged;
		...
		//public event Action<Product_iOS> AppDescriptionChanged;
		...
		//public event Action<Product_iOS> ApplicationIdChanged;
		...
		//public event Action<Product_iOS> EntryPointChanged;
		...
		{
			get { return SystemSettings.Platform.iOS; }
		}
		...
		{
			{
				var p1 = Path.Combine( VirtualFileSystem.Directories.Project, @"Sources\NeoAxis.CoreExtension\NeoAxis.CoreExtension.iOS.csproj" );
			...
			{
				var p1 = Path.Combine( VirtualFileSystem.Directories.Project, @"Project.iOS.csproj" );
		...
		void CopyFilesToPackageFolder( ProductBuildInstance buildInstance )
		{
			...
			//copy Build.iOS.sln or Build.iOS.Extended.sln
			if( File.Exists( Path.Combine( VirtualFileSystem.Directories.Project, "Build.iOS.Extended.sln" ) ) )
			{
				CopyFiles( VirtualFileSystem.Directories.Project, buildInstance.DestinationFolder, buildInstance, new Range( 0.4, 0.4 ), "Build.iOS.Extended.sln" );
			}
			else
			{
				CopyFiles( VirtualFileSystem.Directories.Project, buildInstance.DestinationFolder, buildInstance, new Range( 0.4, 0.4 ), "Build.iOS.sln" );
			}

			//copy Project.iOS.csproj
			CopyFiles( VirtualFileSystem.Directories.Project, buildInstance.DestinationFolder, buildInstance, new Range( 0.4, 0.4 ), "Project.iOS.csproj" );
			...
			//copy Sources\NeoAxis.Player.iOS exclude Project.zip.hash, Project.zip
			{
				var excludePaths = new List<string>();
				excludePaths.Add( @"Sources\NeoAxis.Player.iOS\Resources\Project.zip.hash" );
				excludePaths.Add( @"Sources\NeoAxis.Player.iOS\Resources\Project.zip" );

				CopyFolder( Path.Combine( sourceSourcesPath, "NeoAxis.Player.iOS" ), Path.Combine( destSourcesPath, "NeoAxis.Player.iOS" ), buildInstance, new Range( 0.4, 0.45 ), excludePaths );
			}
			...
			//copy managed dll references from iOS folder
			CopyFiles(
			...
			//create Project.zip
			{
				var destinationFileName = Path.Combine( buildInstance.DestinationFolder, @"Sources\NeoAxis.Player.iOS\Resources\Project.zip" );
