...
namespace NeoAxis
{
	...
	[Editor.AddToResourcesWindow( @"Base\Scene objects\Additional\World Generator", 0 )]
	public class Component_WorldGenerator : Component
	{
			...
			case TemplateEnum.OakForest:
				{
					...
					data.Surfaces.Add( new TemplateData.SurfaceItem( surface1, 5 ) );

					var surface2 = new Reference<Component_Surface>( null, surface2Default );
					data.Surfaces.Add( new TemplateData.SurfaceItem( surface2, 1 ) );
				}
				break;
		...
		void FillGroupOfObjects( Component_Scene scene, Component_Terrain terrain, Component_GroupOfObjects toGroupOfObjects, Random random, Component_Surface surface, int elementIndex )
		{
			...
			{
				var item = new Component_GroupOfObjects.GetObjectsItem( Component_GroupOfObjects.GetObjectsItem.CastTypeEnum.All, null, true, totalBounds );
