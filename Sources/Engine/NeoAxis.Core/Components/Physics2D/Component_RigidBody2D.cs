...
namespace NeoAxis
{
	...
	public class Component_RigidBody2D : Component_PhysicalBody2D
	{
		Body rigidBody;
		...
		public event Action<Component_RigidBody2D> MotionTypeChanged;
		...
		public event Action<Component_RigidBody2D> MassChanged;
		...
		public event Action<Component_RigidBody2D> InertiaChanged;
		...
		public event Action<Component_RigidBody2D> LocalCenterChanged;
		...
		public event Action<Component_RigidBody2D> EnableGravityChanged;
		...
		public event Action<Component_RigidBody2D> LinearDampingChanged;
		...
		public event Action<Component_RigidBody2D> AngularDampingChanged;
		...
		public event Action<Component_RigidBody2D> FixedRotationChanged;
		...
		public event Action<Component_RigidBody2D> AllowSleepChanged;
		...
		public event Action<Component_RigidBody2D> CcdChanged;
		...
		public event Action<Component_RigidBody2D> LinearVelocityChanged;
		...
		public event Action<Component_RigidBody2D> AngularVelocityChanged;
		...
		public event Action<Component_RigidBody2D> ContactsDisplayChanged;
		...
		static void GetComponentShapesRecursive( Component_CollisionShape2D shape, Transform shapeTransform, List<Tuple<Component_CollisionShape2D, Transform>> result )
		{
			result.Add( new Tuple<Component_CollisionShape2D, Transform>( shape, shapeTransform ) );

			foreach( var child in shape.GetComponents<Component_CollisionShape2D>( false, false, true ) )
			{
		...
		List<Component_Constraint2D> GetLinkedCreatedConstraints()
		{
			var list = new List<Component_Constraint2D>();
			...
			if( rigidBody.JointList != null )
			{
				for( var edge = rigidBody.JointList; edge != null; edge = edge.Next )
				{
					...
					var constraint = joint.Tag as Component_Constraint2D;
					if( constraint != null )
						list.Add( constraint );
				}
			}
		...
		void DestroyBody()
		{
			...
			if( physicsWorldData != null )
			{
				...
				if( rigidBody != null )
				{
					...
					physicsWorldData.world.Remove( rigidBody );
					rigidBody = null;
