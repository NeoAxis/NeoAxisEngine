...
namespace NeoAxis
{
	...
	public class Sensor2D : ObjectInSpace
	{
		//Matrix4 hullFrom;
		//Matrix4 hullTo;
		//ShapeEnum hullShape;
		...
		/// </summary>
		[DefaultValue( Physics2DCategories.Category1 )]
		[Category( "Sensor 2D" )]
		...
		ReferenceField<Physics2DCategories> _collisionCategories = Physics2DCategories.Category1;

		/// <summary>
		/// The collision mask bits. This states the categories that this fixture would accept for collision.
		/// </summary>
		[DefaultValue( Physics2DCategories.All )]
		[Category( "Sensor 2D" )]
		...
		public delegate void ObjectsChangedDelegate( Sensor2D sender );
		...
		{
			if( IgnoreSensors && ( item.Object == this || typeof( Sensor2D ).IsAssignableFrom( item.Object.GetType() ) ) )
			{
				ignore = true;
				return;
			}
		...
		public delegate void FilterObjectEventDelegate( Sensor2D sensor, ResultItem item, ref bool ignore );
		...
		public struct ResultItem
		{
			...
			public ResultItem( ObjectInSpace obj, Vector2? position, double distanceScale, Physics2DRayTestItem.ResultItem testResult = null )
			{
				Object = obj;
				Position = position;
		...
		ResultItem[] CalculateObjects_ObjectsInSpace()
		{
			...
			if( scene != null )
			{
				...
				case ShapeEnum.Box:
					{
						var item = new Scene.GetObjectsInSpaceItem( Scene.GetObjectsInSpaceItem.CastTypeEnum.All, null, false, GetBoxExtendedZ() );
				...
				case ShapeEnum.Ray:
					{
						...
						if( ray.Direction != Vector2.Zero )
						{
							...
							var item = new Scene.GetObjectsInSpaceItem( Scene.GetObjectsInSpaceItem.CastTypeEnum.All, null, false, bounds );
							scene.GetObjectsInSpace( item );
		...
		public delegate void CalculateObjectsEventDelegate( Sensor2D sender, ref bool handled, ref ResultItem[] result );
		...
		public delegate void ObjectEnterLeaveDelegate( Sensor2D sender, ObjectInSpace obj );
