...
namespace NeoAxis
{
	...
	public class CollisionShape2D_Ellipse : CollisionShape2D
	{
		/// <summary>
		...
		public event Action<CollisionShape2D_Ellipse> DimensionsChanged;
		...
		public event Action<CollisionShape2D_Ellipse> EdgesChanged;
		...
		public event Action<CollisionShape2D_Ellipse> UseSmoothCircleWhenPossibleChanged;
		...
		protected internal override IList<Fixture> CreateShape( Body body, Transform shapeTransform, List<Vector2> rigidBodyLocalPoints )
		{
			if( shapeTransform.IsPositionZero && shapeTransform.IsRotationIdentity )
			{
				...
				if( Math.Abs( radius.X - radius.Y ) < 0.0001 && UseSmoothCircleWhenPossible )
					return new Fixture[] { body.CreateCircle( (float)radius.X, 0, Physics2DUtility.Convert( Vector2.Zero ) ) };
				else
