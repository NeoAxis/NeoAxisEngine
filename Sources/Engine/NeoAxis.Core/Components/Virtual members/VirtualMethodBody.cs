...
namespace NeoAxis
{
	...
	/// </summary>
	public class VirtualMethodBody : Component, IFlowGraphRepresentationData
	{
		...
		//Definition
		ReferenceField<VirtualMember> _definition;
		[FlowGraphBrowsable( false )]
		public Reference<VirtualMember> Definition
		{
			get
			{
			...
			set
			{
				if( _definition.BeginSet( this, ref value ) )
				{
					try
					{
		...
		public event Action<VirtualMethodBody> DefinitionChanged;
		...
		public Reference<PropertyAccessorTypeEnum> PropertyAccessorType
		{
			...
			set
			{
				if( _propertyAccessorType.BeginSet( this, ref value ) )
				{
					try
					{
		...
		public event Action<VirtualMethodBody> PropertyAccessorTypeChanged;

		//BodyEnd
		ReferenceField<VirtualMethodBodyEnd> _bodyEnd;
		[Serialize]
		[FlowGraphBrowsable( false )]
		public Reference<VirtualMethodBodyEnd> BodyEnd
		{
			get
			{
			...
			set
			{
				if( _bodyEnd.BeginSet( this, ref value ) )
				{
					try
					{
		...
		public event Action<VirtualMethodBody> BodyEndChanged;
		...
		public Reference<FlowInput> Flow
		{
			...
			set
			{
				if( _flow.BeginSet( this, ref value ) )
				{
		...
		public event Action<VirtualMethodBody> FlowChanged;

		/////////////////////////////////////////

		class PropertyImpl : Metadata.Property
		{
			VirtualMethodBody creator;
			string category;
			string displayName;
			...
			public PropertyImpl( object owner, string name, bool isStatic, Metadata.TypeInfo type, Metadata.TypeInfo typeUnreferenced, Metadata.Parameter[] indexers, bool readOnly, VirtualMethodBody creator, string category, string displayName, ParameterType parameterType, int invokeParameterIndex )
		...
		protected override void OnMetadataGetMembersFilter( Metadata.GetMembersContext context, Metadata.Member member, ref bool skip )
		{
			...
			if( p != null )
			{
				...
				case "PropertyAccessorType":
					if( Definition.Value as VirtualProperty == null )
						skip = true;
					break;
				}
			}
