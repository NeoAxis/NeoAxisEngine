...
namespace NeoAxis
{
	...
	public class Component_CurveInSpace : Component_ObjectInSpace
	{
		...
		Component_Mesh mesh;
		Component_MeshInSpace meshInSpace;
		Component_RigidBody collisionBody;
		...
		public Reference<CurveTypeEnum> CurveTypePosition
		{
			...
			set { if( _curveTypePosition.BeginSet( ref value ) ) { try { CurveTypePositionChanged?.Invoke( this ); DataWasChanged(); } finally { _curveTypePosition.EndSet(); } } }
		}
		...
		public Reference<CurveTypeEnum> CurveTypeRotation
		{
			...
			set { if( _curveTypeRotation.BeginSet( ref value ) ) { try { CurveTypeRotationChanged?.Invoke( this ); DataWasChanged(); } finally { _curveTypeRotation.EndSet(); } } }
		}
		...
		public Reference<CurveTypeEnum> CurveTypeScale
		{
			...
			set { if( _curveTypeScale.BeginSet( ref value ) ) { try { CurveTypeScaleChanged?.Invoke( this ); DataWasChanged(); } finally { _curveTypeScale.EndSet(); } } }
		}
		...
		public Reference<double> TimeScale
		{
			...
			set { if( _timeScale.BeginSet( ref value ) ) { try { TimeScaleChanged?.Invoke( this ); DataWasChanged(); } finally { _timeScale.EndSet(); } } }
		}
		...
		public Reference<bool> DisplayCurveInEditor
		{
			...
			set { if( _displayCurveInEditor.BeginSet( ref value ) ) { try { DisplayCurveInEditorChanged?.Invoke( this ); DataWasChanged(); } finally { _displayCurveInEditor.EndSet(); } } }
		}
		...
		public Reference<bool> DisplayCurveInSimulation
		{
			...
			set { if( _displayCurveInSimulation.BeginSet( ref value ) ) { try { DisplayCurveInSimulationChanged?.Invoke( this ); DataWasChanged(); } finally { _displayCurveInSimulation.EndSet(); } } }
		}
		...
		public Reference<bool> Caps
		{
			...
			set { if( _caps.BeginSet( ref value ) ) { try { CapsChanged?.Invoke( this ); DataWasChanged(); } finally { _caps.EndSet(); } } }
		}
		/// <summary>Occurs when the <see cref="Caps"/> property value changes.</summary>
		public event Action<Component_CurveInSpace> CapsChanged;
		...
		public event Action<Component_CurveInSpace> SegmentsLengthChanged;
		...
		public event Action<Component_CurveInSpace> SegmentsCircleChanged;
		...
		public event Action<Component_CurveInSpace> MaterialChanged;
		...
		public event Action<Component_CurveInSpace> UVTilesLengthChanged;
		...
		public event Action<Component_CurveInSpace> UVTilesCircleChanged;
		...
		public event Action<Component_CurveInSpace> UVFlipChanged;
		...
		public event Action<Component_CurveInSpace> VisibilityDistanceChanged;
		...
		public event Action<Component_CurveInSpace> CollisionChanged;
		...
		public event Action<Component_CurveInSpace> CollisionMaterialChanged;
		...
		public event Action<Component_CurveInSpace> CollisionFrictionModeChanged;
		...
		public event Action<Component_CurveInSpace> CollisionFrictionChanged;
		...
		public event Action<Component_CurveInSpace> CollisionAnisotropicFrictionChanged;
		...
		public event Action<Component_CurveInSpace> CollisionSpinningFrictionChanged;
		...
		public event Action<Component_CurveInSpace> CollisionRollingFrictionChanged;
		...
		public event Action<Component_CurveInSpace> CollisionRestitutionChanged;
		...
		protected override void OnMetadataGetMembersFilter( Metadata.GetMembersContext context, Metadata.Member member, ref bool skip )
		{
			...
			if( member is Metadata.Property )
			{
				...
				case nameof( UVFlip ):
				case nameof( VisibilityDistance ):
				...
				case nameof( CollisionAnisotropicFriction ):
					if( Thickness.Value == 0 || !Collision || CollisionFrictionMode.Value == Component_PhysicalMaterial.FrictionModeEnum.Simple || CollisionMaterial.Value != null )
						skip = true;
					break;
		...
		public Curve CreateCurve( CurveTypeEnum type )
		{
			switch( type )
			{
		...
		void UpdateData()
		{
			if( needUpdateData && EnabledInHierarchyAndIsNotResource && GetAllowUpdateData() )
			{
				...
				if( sourceData.Points.Length != 0 )
				{
					...
					data.PositionCurveNormalizedTime = CreateCurve( CurveTypePosition );
					data.ScaleCurveNormalizedTime = CreateCurve( CurveTypeScale );
		...
		protected override void OnComponentAdded( Component component )
		{
			...
			if( component is Component_CurveInSpacePoint )
				DataWasChanged();
		}
		...
		void GenerateMeshData( Component_Mesh mesh, Component_CollisionShape_Mesh collisionShape )
		{
			var data = GetData();
			...
			if( currentIndex != indexCount )
				Log.Fatal( "Component_CurveInSpace: GenerateMeshData: currentIndex != indexCount." );
			foreach( var index in indices )
				if( index < 0 || index >= positions.Count )
					Log.Fatal( "Component_CurveInSpace: GenerateMeshData: index < 0 || index >= positions.Count." );
		...
		void UpdateRenderingObjects()
		{
			...
			if( data != null && data.SourceData != null && data.SourceData.Points.Length > 1 )
			{
				...
				//create mesh in space
				{
					...
					meshInSpace = ComponentUtility.CreateComponent<Component_MeshInSpace>( null, false, false );
					meshInSpace.DisplayInEditor = false;
					AddComponent( meshInSpace, -1 );
					//meshInSpace = CreateComponent<Component_MeshInSpace>();
		...
		void UpdateCollisionObjects()
		{
			...
			var body = ComponentUtility.CreateComponent<Component_RigidBody>( null, false, false );
			body.DisplayInEditor = false;
			AddComponent( body );
			//var group = scene.CreateComponent<Component_GroupOfObjects>();
