...
namespace NeoAxis//Accord.Video.VFW
{
	...
	class AVIWriter : IDisposable
	{
        ...
        /// <exception cref="AVIWriterVideoException">A error occurred while creating new video file. See exception message.</exception>
           ...
           // check width and height
            ...
            try
            {
                lock ( sync )
                {
                    ...
                    // create new file
                    if ( AVIWriterWin32.AVIFileOpen( out file, fileName, AVIWriterWin32.OpenFileMode.Create | AVIWriterWin32.OpenFileMode.Write, IntPtr.Zero ) != 0 )
                    ...
                    // describe new stream
                    AVIWriterWin32.AVISTREAMINFO info = new AVIWriterWin32.AVISTREAMINFO( );

                    info.type    = AVIWriterWin32.mmioFOURCC( "vids" );
                    info.handler = AVIWriterWin32.mmioFOURCC( codec );
                    info.scale   = 1;
                    info.rate    = rate;
                    info.suggestedBufferSize = stride * height;

                    // create stream
                    if ( AVIWriterWin32.AVIFileCreateStream( file, out stream, ref info ) != 0 )
                        throw new AVIWriterVideoException( "Failed creating stream." );

                    // describe compression options
                    AVIWriterWin32.AVICOMPRESSOPTIONS options = new AVIWriterWin32.AVICOMPRESSOPTIONS( );

                    options.handler = AVIWriterWin32.mmioFOURCC( codec );
                    ...
                    if ( AVIWriterWin32.AVIMakeCompressedStream( out streamCompressed, stream, ref options, IntPtr.Zero ) != 0 )
                        throw new AVIWriterVideoException( "Failed creating compressed stream." );

                    // describe frame format
                    AVIWriterWin32.BITMAPINFOHEADER bitmapInfoHeader = new AVIWriterWin32.BITMAPINFOHEADER( );
                    ...
                    // set frame format
                    if ( AVIWriterWin32.AVIStreamSetFormat( streamCompressed, 0, ref bitmapInfoHeader, Marshal.SizeOf( bitmapInfoHeader.GetType( ) ) ) != 0 )
                        throw new AVIWriterVideoException( "Failed setting format of the compressed stream." );
        ...
        public void Close( )
		{
            lock ( sync )
            {
                ...
                {
                    AVIWriterWin32.AVIStreamRelease( streamCompressed );
                ...
                {
                    AVIWriterWin32.AVIStreamRelease( stream );
                    stream = IntPtr.Zero;
                }
                ...
                if ( file != IntPtr.Zero )
                {
                    AVIWriterWin32.AVIFileRelease( file );
                    file = IntPtr.Zero;
                }
            }
		}
